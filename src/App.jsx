    ...
    <div>
      ...
      {res&&(<div style={{marginTop:20}}>
        ...
        {!res.rule14&&(<>
          ...
          {/* Tax impact — depends on classification */}
          <div style={{background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,padding:"12px 14px",marginBottom:10}}>
            <p style={{fontSize:10,fontWeight:700,color:T.gold,margin:"0 0 10px",letterSpacing:"0.08em",textTransform:"uppercase"}}>{"How This Affects Your Tax Return"}</p>

            {/* Vacation home */}
            {res.isVacationHome&&<IBox tone="amber">{"⚠ Vacation Home Rules — personal use exceeds the greater of 14 days or 10% of rental days. Expenses capped at rental income. Loss cannot offset other income. Report on Schedule E."}</IBox>}

            {/* Non-passive: income adds to AGI, losses offset ordinary income */}
            {!res.isVacationHome&&res.isNonPassive&&(<>
              <div style={{padding:"7px 10px",background:"rgba(74,144,96,0.1)",borderRadius:6,marginBottom:8,fontSize:11,color:T.green,fontWeight:600}}>
                {"✅ NON-PASSIVE — material participation qualifies. No SE tax. Report on Schedule E."}
              </div>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                <tbody>
                  <tr style={{borderBottom:"1px solid "+T.border}}>
                  ...
                  <tr style={{borderBottom:"1px solid "+T.border}}>
                    <td style={{padding:"5px 0",color:res.netIncome>=0?T.textMid:T.green}}>
                      {res.netIncome>=0?"+ Net rental income (added to AGI)":"+ Net rental loss (deducted from AGI — no limit)"}
                    </td>
                    <td style={{padding:"5px 0",textAlign:"right",color:res.netIncome>=0?T.gold:T.green}}>
                      {res.netIncome>=0?fmt(res.netIncome):"("+fmt(Math.abs(res.netIncome))+")"}
                    </td>
                  </tr>
                  <tr style={{background:"rgba(200,169,110,0.06)"}}>
                    <td style={{padding:"7px 0",fontWeight:600}}>{"= New taxable income"}</td>
                    <td style={{padding:"7px 0",textAlign:"right",fontWeight:600,color:T.gold}}>{fmt(Math.max(0,res.agi+res.netIncome))}</td>
                  </tr>
                  {res.netIncome>0&&<tr>
                    <td style={{padding:"6px 0",color:T.textMid}}>{"Est. additional tax from rental income"}</td>
                    <td style={{padding:"6px 0",textAlign:"right",color:T.red,fontWeight:600}}>{fmt(res.estimatedIncomeTax)}</td>
                  </tr>}
                </tbody>
              </table>
            </>)}

            {/* Other passive: income adds to AGI, losses carry forward only */}
            {!res.isVacationHome&&!res.isNonPassive&&(<>
              <div style={{padding:"7px 10px",background:"rgba(200,169,110,0.08)",borderRadius:6,marginBottom:8,fontSize:11,color:T.amber,fontWeight:600}}>
                {"⚠ OTHER PASSIVE (STR, no material participation). No SE tax. Report on Schedule E."}
              </div>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                <tbody>
                  <tr style={{borderBottom:"1px solid "+T.border}}>
                  ...
                  {res.netIncome>=0&&<>
                    <tr style={{borderBottom:"1px solid "+T.border}}>
                      <td style={{padding:"5px 0",color:T.textMid}}>{"+ Net rental income (passive, added to AGI)"}</td>
                      <td style={{padding:"5px 0",textAlign:"right",color:T.gold}}>{fmt(res.netIncome)}</td>
                    </tr>
                    <tr style={{background:"rgba(200,169,110,0.06)"}}>
                      <td style={{padding:"7px 0",fontWeight:600}}>{"= New taxable income"}</td>
                      <td style={{padding:"7px 0",textAlign:"right",fontWeight:600,color:T.gold}}>{fmt(res.agi+res.netIncome)}</td>
                    </tr>
                    <tr>
                      <td style={{padding:"6px 0",color:T.textMid}}>{"Est. additional tax from rental income"}</td>
                      <td style={{padding:"6px 0",textAlign:"right",color:T.red,fontWeight:600}}>{fmt(res.estimatedIncomeTax)}</td>
                  ...
                  {res.netIncome<0&&<tr style={{background:"rgba(192,112,80,0.06)"}}>
                    <td colSpan={2} style={{padding:"8px 0"}}>
                      <div style={{fontSize:11,color:T.amber}}>
                        <strong>{"Net loss of "+fmt(Math.abs(res.netIncome))+" — OTHER PASSIVE loss rules:"}</strong><br/>
                        {"• ❌ Cannot offset W-2, business, or ordinary income"}<br/>
                        {"• ❌ $25,000 allowance does NOT apply to STR (long-term rental only)"}<br/>
                        {"• ✓ Can offset other passive income (other rentals, limited partnerships)"}<br/>
                        {"• ✓ Carries forward until passive income exists or property is sold"}<br/>
                        {"• To unlock ordinary income offset: qualify for material participation above"}
                      </div>
                    </td>
                  </tr>}
                </tbody>
