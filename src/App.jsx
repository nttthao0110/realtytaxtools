    ...
    <div>
      ...
      {step===0&&(<div>
        ...
        <SB color="#1a4a3a" tc="#70d0a0" title="D — HUD-1 Expenses Currently Deductible (deduct in year of closing)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            <MF label={"Property Taxes PAID by Buyer at Closing (deductible)"} hint="Taxes you actually PAY at closing — deductible on Schedule E. This is DIFFERENT from the seller tax proration credit in Section F. Do NOT enter seller's unpaid tax credit here." value={inp.propertyTaxClosing} onChange={v=>s("propertyTaxClosing",v)}/>
        ...
        {/* F — Reductions */}
        <div style={{padding:"8px 12px",background:"rgba(200,169,110,0.06)",border:"1px solid "+T.goldDim+"40",borderRadius:6,marginBottom:8}}>
          <p style={{fontSize:10,color:T.gold,fontWeight:600,margin:"0 0 4px"}}>{"⚠ Property Tax — Two Different Things:"}</p>
          <p style={{fontSize:10,color:T.textDim,margin:"0 0 2px"}}>{"Section D above: taxes you PAY → deductible expense"}</p>
          <p style={{fontSize:10,color:T.textDim,margin:0}}>{"Section F below: seller credit for THEIR unpaid taxes → reduces your basis. Enter each in ONE place only."}</p>
        </div>
        <SB color="#5a2a1a" tc="#e09060" title="F — Reductions to Amount Due (all enter as POSITIVE)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            ...
            <MF label={"Tax Proration Credit FROM Seller (unpaid taxes owed by seller)"} hint="Seller credits buyer for property taxes the seller OWE but did not pay. This REDUCES your basis. Enter ONCE here — do NOT also enter in Section D. Example: seller owned Jan–June, closing in July, seller credits you 6 months of taxes." credit value={inp.taxAdjSeller} onChange={v=>s("taxAdjSeller",v)}/>
...
function STRCalc({lang:L="en"}){
  ...
  function doCalc(){
    ...
    // Expense allocation: rental days / total days used
    const rentalPct = total>0 ? rd/total : (rd>0?1:0);
    const deductibleExp = Math.round(exp*rentalPct*100)/100;
    const netIncome = Math.round((inc - deductibleExp)*100)/100; // positive=income, negative=loss

    // 14-day rule: rented ≤14 days → income NOT reportable (§280A(g))
    const rule14 = rd<=14 && rd>0;

    // Vacation home: personal use > greater of 14 days or 10% of rental days
    ...
    // STR classification (avg stay ≤7 days assumed for this calculator):
    // + material participation → NON-PASSIVE: income taxed, losses offset ordinary income
    // + no material participation → OTHER PASSIVE: income taxed, losses carry forward only
    // Vacation home → expenses capped at income, no loss allowed
    const isNonPassive = isMaterialParticipation && !isVacationHome;

    // For ALL cases: compute taxable income including this rental
    // Non-passive income/loss: fully adds/offsets other income
    // Passive income: adds to AGI (taxable)
    // Passive loss: does NOT reduce taxable income (carries forward)
    // Vacation home loss: not deductible at all (capped at income)
    const netIncomeForTax = isVacationHome
      ? Math.max(0, netIncome) // capped at 0 for losses
      : isNonPassive
        ? netIncome // full add or deduct
        : Math.max(0, netIncome); // passive: only income counts; loss doesn't reduce AGI

    // Total taxable income = AGI + rental effect
    const totalTaxableIncome = Math.max(0, agi + netIncomeForTax);

    // Standard deduction (2024)
    const stdDed = STD_DED[inp.fs] || 14600;

    // Tax WITH this rental income/effect
    const taxWithRental = estTax(agi + netIncomeForTax, inp.fs);
    // Tax WITHOUT this rental (baseline)
    const taxWithout = estTax(agi, inp.fs);
    // Additional tax from rental (can be negative if non-passive loss)
    const rentalTaxEffect = taxWithRental - taxWithout;
    // Actual income after std deduction
    const taxableAfterDeduction = Math.max(0, agi + netIncomeForTax - stdDed);

    setRes({rd,pd,inc,exp,agi,rentalPct,deductibleExp,netIncome,
      netIncomeForTax,totalTaxableIncome,taxableAfterDeduction,stdDed,
      taxWithRental,taxWithout,rentalTaxEffect,
      rule14,isVacationHome,isNonPassive,isMaterialParticipation,mpTest,
      total,personalThreshold});
  }
  return(
    <div>
      ...
      <div style={{padding:"10px 14px",background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,marginBottom:8}}>
        ...
        <Sel value={inp.mpTest} onChange={v=>s("mpTest",v)} options={[
          ...
          {value:"sig500",label:"✅  Test 4 — Significant participation activities total 500+ hrs (all combined)"},
      ...
      {res&&(<div style={{marginTop:20}}>
        ...
        {!res.rule14&&(<>
          ...
          <div style={{background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,padding:"12px 14px",marginBottom:10}}>
            <p style={{fontSize:10,fontWeight:700,color:T.gold,margin:"0 0 10px",letterSpacing:"0.08em",textTransform:"uppercase"}}>{"Rental Income Breakdown"}</p>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <tbody>
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  <td style={{padding:"5px 0",color:T.textMid}}>{"Gross rental income"}</td>
                  <td style={{padding:"5px 0",textAlign:"right"}}>{fmt(res.inc)}</td>
                </tr>
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  <td style={{padding:"5px 0",color:T.textMid,paddingLeft:12}}>{"− Deductible expenses ("}{pct(res.rentalPct)}{" rental use)"}</td>
                  <td style={{padding:"5px 0",textAlign:"right",color:T.green}}>{"("+fmt(res.deductibleExp)+")"}</td>
                </tr>
                <tr style={{background:"rgba(200,169,110,0.06)"}}>
                  <td style={{padding:"7px 0",fontWeight:600,color:res.netIncome>=0?T.gold:T.red}}>
                    {res.netIncome>=0?"= Net rental income":"= Net rental loss"}
                  </td>
          ...
          <div style={{padding:"7px 12px",borderRadius:6,marginBottom:10,fontSize:11,fontWeight:600,
          ...
          <div style={{background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,padding:"12px 14px",marginBottom:10}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <tbody>
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  <td style={{padding:"5px 0",color:T.textMid}}>{"Other income (AGI before this rental)"}</td>
                  <td style={{padding:"5px 0",textAlign:"right"}}>{fmt(res.agi)}</td>
                </tr>
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  <td style={{padding:"5px 0",color:T.textMid}}>
                    {res.isVacationHome&&res.netIncome<0&&"Vacation home loss (not deductible — $0 effect)"}
                    {res.isVacationHome&&res.netIncome>=0&&"+ Rental income (vacation home)"}
                    {!res.isVacationHome&&res.isNonPassive&&res.netIncome>=0&&"+ Net rental income (non-passive, adds to AGI)"}
                    {!res.isVacationHome&&res.isNonPassive&&res.netIncome<0&&"− Net rental loss (non-passive, offsets ordinary income)"}
                    {!res.isVacationHome&&!res.isNonPassive&&res.netIncome>=0&&"+ Net rental income (passive, adds to AGI)"}
                    {!res.isVacationHome&&!res.isNonPassive&&res.netIncome<0&&"Passive loss (carries forward — does NOT reduce taxable income)"}
                  </td>
                  <td style={{padding:"5px 0",textAlign:"right",
                    ...
                    {(!res.isVacationHome&&!res.isNonPassive&&res.netIncome<0)
                      ? "$0 (loss carried forward)"
                      : res.netIncome>=0?fmt(res.netIncome):"("+fmt(Math.abs(res.netIncome))+")"}
                  </td>
                </tr>
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  <td style={{padding:"6px 0",color:T.textMid,fontWeight:500}}>{"= Adjusted Gross Income (AGI)"}</td>
                  <td style={{padding:"6px 0",textAlign:"right",fontWeight:500}}>{fmt(res.totalTaxableIncome)}</td>
                </tr>
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  <td style={{padding:"5px 0",color:T.textMid,paddingLeft:12}}>{"− Standard deduction ("+(inp.fs==="mfj"?"MFJ $29,200":inp.fs==="hoh"?"HOH $21,900":"Single $14,600")+")"}</td>
          ...
          <div style={{background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,padding:"12px 14px",marginBottom:10}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <tbody>
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  <td style={{padding:"5px 0",color:T.textMid}}>{"Est. tax without this rental"}</td>
                ...
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  <td style={{padding:"5px 0",color:res.rentalTaxEffect>=0?T.red:T.green}}>
                    {res.rentalTaxEffect>=0?"+ Tax from rental income":"− Tax savings from rental loss"}
                  </td>
            ...
            {(!res.isVacationHome&&!res.isNonPassive&&res.netIncome<0)&&(
              <div style={{marginTop:8,padding:"6px 10px",background:"rgba(192,112,80,0.06)",borderRadius:5,fontSize:11,color:T.amber}}>
                {"⚠ Passive loss of "+fmt(Math.abs(res.netIncome))+" carried forward. No tax benefit until: (1) future passive income, or (2) property sold. To unlock: qualify for material participation above."}
              </div>
            )}
            <p style={{fontSize:9,color:T.textDim,marginTop:8,lineHeight:1.5,fontStyle:"italic"}}>
              {"Federal estimate only using standard deduction. State taxes not included. SE tax does not apply to typical STR (Schedule E). Consult a CPA for your situation."}
            </p>
          </div>
        </>)}
        <IBox>{"📋 STR losses reported on Schedule E. No SE tax for typical vacation rental. Consult a CPA if you provide substantial services."}</IBox>
        <p style={{fontSize:10,color:T.textDim,marginTop:10,lineHeight:1.5}}>{"Federal estimate only. State taxes not included. Not tax advice."}</p>
      </div>)}
    </div>
  );
}
