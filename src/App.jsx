...
function DepreciationCalc({lang:L="en"}){
  ...
  const [inp,setInp]=useState({
    purchasePrice:"",
    // B — Added to basis
    titleInsurance:"",ownersTitleInsurance:"",recordingFees:"",transferTax:"",
    surveyFee:"",inspectionFee:"",settlementFee:"",otherBasis:"",
    // C — Loan costs (GROSS — lender credit shown separately in reductions)
    originationFee:"",discountPoints:"",appraisalFee:"",creditReport:"",
    mortgageInsurance:"",assumptionFee:"",attorneyFeeLoan:"",lenderOther:"",lenderCredit:"",
    // D — Prepaids (currently deductible)
    prepaidInterest:"",prepaidInsurance:"",prepaidTax:"",
    // E — Reserves/escrow
    escrowDeposit:"",securityDeposit:"",
    // Reductions to amount due
    earnestMoney:"",loanAmount:"",sellerCredit:"",
    unpaidTaxSeller:"",   // tax adj unpaid by seller — adds to basis AND is reduction
    unpaidAssessment:"",  // other unpaid items assumed — adds to basis only
    proratedTaxDeductible:"",  // prorated tax credit — DEDUCTIBLE only, not a cash reduction
    proratedRentIncome:"",     // prorated rent — taxable income Schedule E
    proratedHOA:"",
    otherReduction:"",
    // Land
  ...
  // ── Derived ────────────────────────────────────────────────────────────────
  // B — Basis closing costs
  const basisCC=
    parse(inp.titleInsurance)+parse(inp.ownersTitleInsurance)+
    parse(inp.recordingFees)+parse(inp.transferTax)+
    parse(inp.surveyFee)+parse(inp.inspectionFee)+
    parse(inp.settlementFee)+parse(inp.otherBasis);

  // Unpaid items assumed — tax adj adds to BASIS and is a REDUCTION
  // Other special assessments add to BASIS only
  const unpaidTaxSeller=parse(inp.unpaidTaxSeller);
  const unpaidAssessment=parse(inp.unpaidAssessment);
  const totalUnpaidAddedToBasis=unpaidTaxSeller+unpaidAssessment;
  const totalBasis=parse(inp.purchasePrice)+basisCC+totalUnpaidAddedToBasis;

  // C — Gross loan costs (GROSS shown in charges — lender credit shown in reductions separately)
  const grossLoanCosts=hasloan
    ?(parse(inp.originationFee)+parse(inp.discountPoints)+parse(inp.appraisalFee)+
      parse(inp.creditReport)+parse(inp.mortgageInsurance)+parse(inp.assumptionFee)+
      parse(inp.attorneyFeeLoan)+parse(inp.lenderOther))
    :0;
  const lenderCreditAmt=parse(inp.lenderCredit);
  const netLoanCosts=Math.max(0,grossLoanCosts-lenderCreditAmt);

  // D — Prepaids
  const prepaidsTotal=parse(inp.prepaidInterest)+parse(inp.prepaidInsurance)+parse(inp.prepaidTax);
  // Currently deductible = prepaid interest + prepaid tax + prorated tax (deductible but NOT a cash reduction)
  const currentYearDeductible=parse(inp.prepaidInterest)+parse(inp.prepaidTax)+parse(inp.proratedTaxDeductible);

  // E — Reserves/escrow
  const escrowTotal=parse(inp.escrowDeposit)+parse(inp.securityDeposit);

  // Reconciliation — match HUD-1 exactly
  // CHARGES to borrower (everything borrower pays)
  const totalCharges=parse(inp.purchasePrice)+basisCC+grossLoanCosts+prepaidsTotal+escrowTotal;

  // REDUCTIONS to amount due (everything that reduces what borrower brings)
  // Note: proratedTaxDeductible is a TAX DEDUCTION for borrower but is NOT a cash reduction
  // (seller already paid the tax through closing date — buyer gets deduction when they pay later)
  // unpaidTaxSeller IS a cash reduction (seller gives credit) AND adds to basis
  const loanAmt=parse(inp.loanAmount);
  const earnestAmt=parse(inp.earnestMoney);
  const totalReductions=
    earnestAmt+loanAmt+parse(inp.sellerCredit)+lenderCreditAmt+
    unpaidTaxSeller+   // tax adj unpaid by seller — IS a cash reduction
    parse(inp.proratedHOA)+parse(inp.proratedRentIncome)+parse(inp.otherReduction);
  const calcCashToClose=Math.max(0,totalCharges-totalReductions);

  // Land
  const landRatio=inp.landRatioMode==="direct"?Math.min(1,Math.max(0,parse(inp.landRatioDirect)/100)):(parse(inp.totalAssessed)>0?Math.min(1,parse(inp.landAssessed)/parse(inp.totalAssessed)):0);
  ...
  const classes=[
    {key:"alloc27",label:"Residential structure (27.5 yr)",basis:inp.propType==="residential"?parse(inp.alloc27)||Math.max(0,buildingBasis-parse(inp.alloc15)-parse(inp.alloc7)-parse(inp.alloc5)):0,life:27.5,isReal:true,color:"#4a9060",show:inp.propType==="residential"},
    {key:"alloc39",label:"Non-residential structure (39 yr)",basis:inp.propType==="nonresidential"?parse(inp.alloc39)||Math.max(0,buildingBasis-parse(inp.alloc15)-parse(inp.alloc7)-parse(inp.alloc5)):0,life:39,isReal:true,color:"#4a6fa8",show:inp.propType==="nonresidential"},
  ...
  async function handlePDF(e){
    ...
    try{
      ...
      const prompt=`You are a CPA analyzing a real estate closing statement. Extract and categorize every line item. Items paid outside of closing are NOT included unless listed on the statement. Only items listed on the statement are captured. Return ONLY valid JSON:\n{"documentType":"HUD-1 or Closing Disclosure or Other","hasloan":true,"purchasePrice":null,"addedToBasis":{"titleInsurance":null,"ownersTitleInsurance":null,"recordingFees":null,"transferTax":null,"deedStamps":null,"attorneyFeesBasis":null,"settlementFee":null,"surveyFee":null,"inspectionFee":null,"otherBasis":null,"otherBasisDesc":""},"loanCosts":{"originationFee":null,"discountPoints":null,"appraisalFee":null,"creditReport":null,"floodCert":null,"underwritingFee":null,"processingFee":null,"taxServiceFee":null,"lendersTitleInsurance":null,"otherLoanCost":null,"otherLoanDesc":"","lenderCredit":null},"prepaids":{"prepaidInterest":null,"prepaidInsurance":null,"prepaidTax":null},"escrow":{"escrowInsurance":null,"escrowTax":null,"escrowMortgageIns":null},"sellerCredits":{"sellerCredit":null,"sellerPaidClosingCosts":null,"proratedTaxCredit":null,"proratedRentCredit":null,"proratedHOACredit":null,"otherCredit":null},"unpaidAssumed":{"unpaidTaxAssumed":null,"unpaidAssessment":null},"financing":{"loanAmount":null,"downPayment":null,"cashToClose":null},"flags":["one-line note ONLY for unusual items needing CPA verify — max 5 — be concise"],"unidentified":["item and amount you could not categorize"]}`;
      const response=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:2000,messages:[{role:"user",content:[{type:"document",source:{type:"base64",media_type:"application/pdf",data:base64}},{type:"text",text:prompt}]}]})});
      const data=await response.json();
      const d=JSON.parse(data.content.map(i=>i.text||"").join("").replace(/```json|```/g,"").trim());
      setHasloan(d.hasloan!==false);
      if(d.purchasePrice)s("purchasePrice",String(d.purchasePrice));
      const ab=d.addedToBasis||{};
      if(ab.titleInsurance)s("titleInsurance",String(ab.titleInsurance));
      if(ab.ownersTitleInsurance)s("ownersTitleInsurance",String(ab.ownersTitleInsurance));
      ...
      if(ab.attorneyFeesBasis)s("attorneyFeesBasis",String(ab.attorneyFeesBasis));
      if(ab.settlementFee)s("settlementFee",String(ab.settlementFee));
      if(ab.surveyFee)s("surveyFee",String(ab.surveyFee));
      if(ab.inspectionFee)s("inspectionFee",String(ab.inspectionFee));
      if(ab.otherBasis)s("otherBasis",String(ab.otherBasis));
      const lc=d.loanCosts||{};
      ...
      if(lc.appraisalFee)s("appraisalFee",String(lc.appraisalFee));
      if(lc.creditReport)s("creditReport",String(lc.creditReport));
      if(lc.floodCert)s("mortgageInsurance",String(lc.floodCert));
      if(lc.underwritingFee)s("assumptionFee",String(lc.underwritingFee));
      if(lc.processingFee)s("attorneyFeeLoan",String(lc.processingFee));
      if(lc.otherLoanCost)s("lenderOther",String(lc.otherLoanCost));
      if(lc.lenderCredit)s("lenderCredit",String(lc.lenderCredit));
      const pp=d.prepaids||{};
      if(pp.prepaidInterest)s("prepaidInterest",String(pp.prepaidInterest));
      if(pp.prepaidInsurance)s("prepaidInsurance",String(pp.prepaidInsurance));
      if(pp.prepaidTax)s("prepaidTax",String(pp.prepaidTax));
      const esc=d.escrow||{};
      if(esc.escrowInsurance)s("escrowDeposit",String(esc.escrowInsurance));
      if(esc.escrowTax)s("securityDeposit",String(esc.escrowTax));
      const sc=d.sellerCredits||{};
      if(sc.sellerCredit)s("sellerCredit",String(sc.sellerCredit));
      if(sc.proratedTaxCredit)s("proratedTaxDeductible",String(sc.proratedTaxCredit));
      if(sc.proratedRentCredit)s("proratedRentIncome",String(sc.proratedRentCredit));
      if(sc.proratedHOACredit)s("proratedHOA",String(sc.proratedHOACredit));
      if(sc.otherCredit)s("otherReduction",String(sc.otherCredit));
      const ua=d.unpaidAssumed||{};
      if(ua.unpaidTaxAssumed)s("unpaidTaxSeller",String(ua.unpaidTaxAssumed));
      if(ua.unpaidAssessment)s("unpaidAssessment",String(ua.unpaidAssessment));
      const fin=d.financing||{};
      if(fin.loanAmount)s("loanAmount",String(fin.loanAmount));
      if(fin.downPayment)s("earnestMoney",String(fin.downPayment));
      const flags=[];
      if(d.flags?.length>0)flags.push(...d.flags);
      if(d.unidentified?.length>0)d.unidentified.forEach(u=>flags.push("Unidentified: "+u));
      setPdfFlags(flags);
      setPdfLoading(false);
    }catch(err){
      setPdfFlags(["Could not read PDF — please enter manually below."]);
      setPdfLoading(false);
    }
  }

  function SRow({label,value,color,bold,indent}){
    if(!value||value==="$0")return null;
    return(
      <tr>
        <td style={{padding:"5px 0",color:bold?T.gold:T.textMid,fontWeight:bold?700:400,fontSize:bold?13:12,paddingLeft:indent?16:0}}>{label}</td>
        <td style={{padding:"5px 0",textAlign:"right",color:color||T.text,fontWeight:bold?700:400,fontSize:bold?13:12}}>{value}</td>
      </tr>
    );
  }
  function Divrow(){return(<tr><td colSpan={2}><div style={{borderTop:"1px solid "+T.border,margin:"4px 0"}}/></td></tr>);}
  function SecLbl({text,color}){return <p style={{fontSize:10,color:color||T.gold,margin:"0 0 8px",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase"}}>{text}</p>;}
  function SecBox({color,children}){return <div style={{padding:"12px 14px",background:T.bgCard2,borderRadius:8,border:"1px solid "+(color||T.border)+"30",marginBottom:10}}>{children}</div>;}
  function MF({label,field,hint}){
    return(
      <div style={{marginBottom:8}}>
        <div style={{fontSize:10,color:T.textMid,marginBottom:3,textTransform:"uppercase",letterSpacing:"0.04em"}}>{label}</div>
  ...
  function calculate(){
    setRes({totalBasis,basisCC,unpaidAssumed,grossLoanCosts,netLoanCosts,lenderCreditAmt,prepaidsTotal,currentYearDeductible,escrowTotal,calcCashToClose,totalCharges,landBasis,buildingBasis,landRatio,classResults,totalAnnual,totalFirstYear,totalBonus,inp,hasloan});
    setStep(3);
  }
  function resetAll(){
    setStep(0);setRes(null);setPdfFlags([]);setHasloan(true);
    setInp({purchasePrice:"",titleInsurance:"",ownersTitleInsurance:"",recordingFees:"",transferTax:"",surveyFee:"",inspectionFee:"",settlementFee:"",otherBasis:"",originationFee:"",discountPoints:"",appraisalFee:"",creditReport:"",mortgageInsurance:"",assumptionFee:"",attorneyFeeLoan:"",lenderOther:"",lenderCredit:"",prepaidInterest:"",prepaidInsurance:"",prepaidTax:"",escrowDeposit:"",securityDeposit:"",earnestMoney:"",loanAmount:"",sellerCredit:"",unpaidTaxSeller:"",unpaidAssessment:"",proratedTaxDeductible:"",proratedRentIncome:"",proratedHOA:"",otherReduction:"",landRatioMode:"assessed",landAssessed:"",totalAssessed:"",landRatioDirect:"",propType:"residential",alloc27:"",alloc39:"",alloc15:"",alloc7:"",alloc5:"",bonusDepreciation:true,bonusPct:"60",yearPlaced:new Date().getFullYear().toString(),monthPlaced:"1"});
  }
  ...
  return(
    <div>
      <div style={{marginBottom:14}}>
        ...
        <p style={{margin:0,fontSize:12,color:T.textDim}}>{"Full HUD-1/CD analysis with cash-to-close reconciliation and depreciation schedule"}</p>
      </div>
      ...
      {step===0&&(<div>
        <Card>
          <CardTitle>{"Upload Closing Statement (PDF)"}</CardTitle>
          <CardSub>{"HUD-1 or Closing Disclosure — Claude reads every line and auto-fills below. Only items listed on the statement are captured."}</CardSub>
          <div style={{padding:"16px",background:T.bgCard2,border:"2px dashed "+(pdfLoading?T.gold:T.border),borderRadius:8,textAlign:"center",marginBottom:10,transition:"border-color 0.3s"}}>
            <div style={{fontSize:24,marginBottom:6}}>{pdfLoading?"🤖":"📄"}</div>
            <p style={{fontSize:12,color:T.textMid,margin:"0 0 10px"}}>{pdfLoading?"Claude is analyzing every line item...":"Upload to auto-fill all fields"}</p>
            {!pdfLoading&&(
              <label style={{display:"inline-block",padding:"8px 18px",borderRadius:6,fontSize:12,fontFamily:"inherit",cursor:"pointer",fontWeight:600,background:"linear-gradient(135deg,"+T.gold+","+T.goldLight+")",color:"#050608"}}>
          ...
          {pdfFlags.length>0&&(
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              {pdfFlags.map((f,i)=>(
                <div key={i} style={{fontSize:11,color:f.startsWith("Unidentified")?T.red:T.textMid,padding:"4px 10px",background:f.startsWith("Unidentified")?T.redBg:T.bgCard2,borderRadius:4,lineHeight:1.5}}>
                  {f.startsWith("Unidentified")?"⚠ "+f:"📝 "+f}
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{marginBottom:10,padding:"10px 14px",background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8}}>
          <Toggle value={hasloan} onChange={setHasloan} label={"Has Mortgage / Loan"} desc={"Turn off for cash purchases — loan cost section hidden"}/>
        </div>

        <SecBox color="#4a9060">
          <SecLbl text={"A — Purchase Price"} color="#4a9060"/>
          <MF label={"Purchase price (from contract)"} field="purchasePrice"/>
        </SecBox>

        <SecBox color="#4a6fa8">
          <SecLbl text={"B — HUD-1 Expenses that Increase Property Cost Basis"} color="#4a6fa8"/>
          <p style={{fontSize:10,color:T.textDim,margin:"0 0 8px",fontStyle:"italic"}}>{"Capitalized — depreciated over life of property. Do NOT include items paid outside of closing (POC)."}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Title Insurance - Lender"} field="titleInsurance"/>
            <MF label={"Title Insurance - Owner"} field="ownersTitleInsurance"/>
            <MF label={"Title Search / other title fees"} field="settlementFee"/>
            <MF label={"Gov't recording charges"} field="recordingFees"/>
            <MF label={"State/City/County tax stamps"} field="transferTax"/>
            <MF label={"Survey"} field="surveyFee"/>
            <MF label={"Inspection(s)"} field="inspectionFee"/>
            <MF label={"Other fees (basis)"} field="otherBasis" hint="NOT items paid outside of closing"/>
          </div>
          {basisCC>0&&<div style={{marginTop:8,padding:"6px 10px",background:"rgba(74,111,168,0.1)",border:"1px solid #2a3060",borderRadius:5,fontSize:12,color:"#6a8ac0"}}>Total closing (basis): <strong>{fmt(basisCC)}</strong> · Property cost basis: <strong style={{color:T.gold}}>{fmt(parse(inp.purchasePrice)+basisCC)}</strong></div>}
        </SecBox>

        {hasloan&&(<SecBox color="#9a7a60">
          <SecLbl text={"C — HUD-1 Expenses that Increase Loan Cost Basis"} color="#9a7a60"/>
          <p style={{fontSize:10,color:T.textDim,margin:"0 0 8px",fontStyle:"italic"}}>{"Amortized over loan term. NOT added to property basis. Lender credit entered separately below."}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Loan origination fee"} field="originationFee"/>
            <MF label={"Loan discount (points)"} field="discountPoints" hint="May be immediately deductible if primary home"/>
            <MF label={"Appraisal (Required by Lender)"} field="appraisalFee" hint="Only if listed on statement — NOT if paid outside closing"/>
            <MF label={"Credit report"} field="creditReport"/>
            <MF label={"Mortgage insurance premium"} field="mortgageInsurance"/>
            <MF label={"Assumption / application fee"} field="assumptionFee"/>
            <MF label={"Attorney fees (listed under loan costs)"} field="attorneyFeeLoan" hint="Attorney fees in the loan cost section go here"/>
            <MF label={"Lender other charges"} field="lenderOther"/>
          </div>
          <div style={{borderTop:"1px solid #4a3020",marginTop:10,paddingTop:10}}>
            <MF label={"Lender credit to buyer (enter in Reductions section below)"} field="lenderCredit" hint="Shown as reduction to amount due — offsets loan costs"/>
          </div>
          {grossLoanCosts>0&&(
            <div style={{marginTop:8,padding:"6px 10px",background:"rgba(154,122,96,0.1)",border:"1px solid #4a3020",borderRadius:5,fontSize:12,color:"#9a7a60"}}>
              Total loan cost basis: <strong>{fmt(grossLoanCosts)}</strong>
              {lenderCreditAmt>0&&<span> · After lender credit: <strong style={{color:T.gold}}>{fmt(netLoanCosts)}</strong> net to amortize</span>}
            </div>
          )}
        </SecBox>)}
        </SecBox>)}
        ...
        <SecBox color="#4a9060">
          <SecLbl text={"D — HUD-1 Expenses Currently Deductible"} color="#4a9060"/>
          <p style={{fontSize:10,color:T.textDim,margin:"0 0 8px",fontStyle:"italic"}}>{"Deductible in the year of closing. Only items listed on the closing statement — not items paid outside closing."}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Property taxes paid ← DEDUCTIBLE"} field="prepaidTax" hint="Property tax paid at closing"/>
            <MF label={"Interest paid (prepaid) ← DEDUCTIBLE"} field="prepaidInterest" hint="Interest from closing date to end of month"/>
            <MF label={"Insurance paid (not immediately deductible)"} field="prepaidInsurance" hint="Prorated over policy term"/>
          </div>
          {(parse(inp.prepaidInterest)+parse(inp.prepaidTax))>0&&<div style={{marginTop:8,padding:"6px 10px",background:"rgba(74,144,96,0.08)",border:"1px solid #1a4020",borderRadius:5,fontSize:12,color:T.green}}>Currently deductible: <strong>{fmt(parse(inp.prepaidInterest)+parse(inp.prepaidTax))}</strong></div>}
        </SecBox>

        <SecBox color="#7a6a9a">
          <SecLbl text={"E — Reserves and Deposits (not yet deductible)"} color="#7a6a9a"/>
          <p style={{fontSize:10,color:T.textDim,margin:"0 0 8px",fontStyle:"italic"}}>{"Deductible only when actually paid out from escrow — not when deposited."}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Deposit with lender (escrow impound)"} field="escrowDeposit"/>
            <MF label={"Security deposit from seller"} field="securityDeposit"/>
          </div>
        </SecBox>

        <SecBox color="#c8a96e">
          <SecLbl text={"Reductions to Amount Due"} color="#c8a96e"/>
          <p style={{fontSize:10,color:T.textDim,margin:"0 0 8px",fontStyle:"italic"}}>{"These reduce what the borrower brings to closing."}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Earnest money deposit"} field="earnestMoney" hint="Already paid before closing"/>
            <MF label={"Loan funds"} field="loanAmount" hint="From lender"/>
            <MF label={"Seller credit"} field="sellerCredit" hint="Does NOT reduce your property basis"/>
            <MF label={"Tax adjustment unpaid by seller"} field="unpaidTaxSeller" hint="Also adds to your property basis"/>
            <MF label={"Prorated HOA credit"} field="proratedHOA"/>
            <MF label={"Prorated rent received (taxable — Sch. E)"} field="proratedRentIncome"/>
            <MF label={"Other reductions"} field="otherReduction"/>
          </div>
          <div style={{borderTop:"1px solid #6a5020",marginTop:10,paddingTop:10}}>
            <MF label={"Prorated property tax credit ← DEDUCTIBLE (not a cash reduction to closing)"} field="proratedTaxDeductible" hint="Tax deduction for buyer — does not reduce cash to close"/>
          </div>
          {parse(inp.unpaidTaxSeller)>0&&<div style={{marginTop:8,padding:"6px 10px",background:"rgba(200,169,110,0.08)",border:"1px solid "+T.goldDim,borderRadius:5,fontSize:11,color:T.goldDim}}>
            Tax adj. unpaid by seller: adds <strong>{fmt(parse(inp.unpaidTaxSeller))}</strong> to basis AND reduces cash to close
          </div>}
        </SecBox>
        ...
        {parse(inp.purchasePrice)>0&&(
          <div style={{background:"linear-gradient(135deg,#0a1a0e,#080e0a)",border:"1px solid #1a4020",borderRadius:10,padding:"16px 18px",marginBottom:14}}>
            <p style={{fontSize:10,color:"#2a6030",margin:"0 0 12px",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase"}}>{"HUD-1 Reconciliation — Cash Due from Borrower"}</p>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <tbody>
                <tr><td colSpan={2} style={{fontSize:10,color:"#4a8070",padding:"2px 0 6px",fontWeight:600,letterSpacing:"0.06em",textTransform:"uppercase"}}>{"Charges to Borrower"}</td></tr>
                <SRow label={"Purchase price"} value={fmt(parse(inp.purchasePrice))}/>
                {basisCC>0&&<SRow label={"HUD-1 expenses — property cost basis (B)"} value={fmt(basisCC)} color="#4a8fc0" indent/>}
                {hasloan&&grossLoanCosts>0&&<SRow label={"HUD-1 expenses — loan cost basis (C)"} value={fmt(grossLoanCosts)} color="#9a7a60" indent/>}
                {prepaidsTotal>0&&<SRow label={"HUD-1 expenses currently deductible (D)"} value={fmt(prepaidsTotal)} color={T.green} indent/>}
                {escrowTotal>0&&<SRow label={"Reserves and deposits (E)"} value={fmt(escrowTotal)} color="#7a6a9a" indent/>}
                <Divrow/>
                <SRow label={"Gross amount due from borrower"} value={fmt(totalCharges)} bold/>
                <Divrow/>
                <tr><td colSpan={2} style={{fontSize:10,color:"#4a8070",padding:"6px 0 4px",fontWeight:600,letterSpacing:"0.06em",textTransform:"uppercase"}}>{"Reductions to Amount Due"}</td></tr>
                {earnestAmt>0&&<SRow label={"Earnest money deposit"} value={"("+fmt(earnestAmt)+")"} color={T.green} indent/>}
                {hasloan&&loanAmt>0&&<SRow label={"Loan funds"} value={"("+fmt(loanAmt)+")"} color={T.green} indent/>}
                {parse(inp.sellerCredit)>0&&<SRow label={"Seller credit"} value={"("+fmt(parse(inp.sellerCredit))+")"} color={T.green} indent/>}
                {lenderCreditAmt>0&&<SRow label={"Lender credit to buyer"} value={"("+fmt(lenderCreditAmt)+")"} color={T.green} indent/>}
                {unpaidTaxSeller>0&&<SRow label={"Tax adjustment unpaid by seller"} value={"("+fmt(unpaidTaxSeller)+")"} color={T.green} indent/>}
                {parse(inp.proratedHOA)>0&&<SRow label={"Prorated HOA"} value={"("+fmt(parse(inp.proratedHOA))+")"} color={T.green} indent/>}
                {parse(inp.proratedRentIncome)>0&&<SRow label={"Prorated rent"} value={"("+fmt(parse(inp.proratedRentIncome))+")"} color={T.green} indent/>}
                {parse(inp.otherReduction)>0&&<SRow label={"Other reductions"} value={"("+fmt(parse(inp.otherReduction))+")"} color={T.green} indent/>}
                <Divrow/>
                <SRow label={"Cash due from borrower (Line 303)"} value={fmt(calcCashToClose)} bold color={T.gold}/>
              </tbody>
            </table>
            <div style={{marginTop:8,fontSize:11,color:"#3a7050",lineHeight:1.5}}>
              {"✓ Compare to Line 303 on your HUD-1. If it matches, all line items are correctly captured."}
            </div>
          </div>
        )}
        ...
        {totalBasis>0&&(
          <div style={{background:T.bgCard2,border:"1px solid "+T.border,borderRadius:10,padding:"14px 16px",marginBottom:14}}>
            <p style={{fontSize:10,color:T.gold,margin:"0 0 10px",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase"}}>{"Tax Treatment Summary"}</p>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <tbody>
                <SRow label={"Property cost basis"} value={fmt(totalBasis)} bold color={T.gold}/>
                <SRow label={"  Purchase price"} value={fmt(parse(inp.purchasePrice))} color={T.textMid} indent/>
                {basisCC>0&&<SRow label={"  + Capitalized closing costs (B)"} value={fmt(basisCC)} color={T.textMid} indent/>}
                {unpaidTaxSeller>0&&<SRow label={"  + Tax adjustment unpaid by seller"} value={fmt(unpaidTaxSeller)} color={T.textMid} indent/>}
                {unpaidAssessment>0&&<SRow label={"  + Unpaid special assessment"} value={fmt(unpaidAssessment)} color={T.textMid} indent/>}
                <Divrow/>
                {currentYearDeductible>0&&<SRow label={"Currently deductible (D + prorated tax)"} value={fmt(currentYearDeductible)} bold color={T.green}/>}
                {(parse(inp.prepaidInterest)+parse(inp.prepaidTax))>0&&<SRow label={"  Prepaid interest + property tax (D)"} value={fmt(parse(inp.prepaidInterest)+parse(inp.prepaidTax))} color={T.green} indent/>}
                {parse(inp.proratedTaxDeductible)>0&&<SRow label={"  Prorated property tax credit"} value={fmt(parse(inp.proratedTaxDeductible))} color={T.green} indent/>}
                <Divrow/>
                {netLoanCosts>0&&<SRow label={"Loan cost basis (amortize over loan term)"} value={fmt(netLoanCosts)} bold color="#9a7a60"/>}
                {escrowTotal>0&&<SRow label={"Escrow deposits (deduct when paid out)"} value={fmt(escrowTotal)} color="#7a6a9a"/>}
                {parse(inp.proratedRentIncome)>0&&<SRow label={"Prorated rent received (taxable — Sch. E)"} value={fmt(parse(inp.proratedRentIncome))} color={T.red}/>}
              </tbody>
            </table>
          </div>
        )}
      ...
      {step===1&&(<Card>
        ...
        <CardSub>{"Land is not depreciable — separate it from total basis"}</CardSub>
        <IBox>{"💡 Find at county assessor website. Search: county assessor property search + your address"}</IBox>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,margin:"12px 0"}}>
          {[{v:"assessed",l:"Enter Assessed Values",d:"Land + total → ratio calculated"},{v:"direct",l:"Enter % Directly",d:"You know the land %"}].map(o=>(
            <button key={o.v} onClick={()=>s("landRatioMode",o.v)} style={{padding:"10px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",textAlign:"left",transition:"all 0.15s",background:inp.landRatioMode===o.v?"rgba(200,169,110,0.12)":T.bgCard,border:"1px solid "+(inp.landRatioMode===o.v?T.gold:T.border)}}>
        ...
        {inp.landRatioMode==="assessed"&&(<>
          <Fld label={"Assessed land value"} hint={"From county assessor"}><Inp value={inp.landAssessed} onChange={v=>s("landAssessed",v)} placeholder="0" prefix="$"/></Fld>
        ...
        {inp.landRatioMode==="direct"&&(
          <Fld label={"Land percentage (%)"} hint={"Enter 25 if land = 25% of total"}><Inp value={inp.landRatioDirect} onChange={v=>s("landRatioDirect",v)} placeholder="25"/></Fld>
        )}
      ...
      {step===2&&(<Card>
        ...
        <CardSub>{"Allocate building basis into asset classes — shorter classes depreciate faster (Cost Segregation)"}</CardSub>
        <Fld label={"Property type"}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[{v:"residential",l:"Residential rental — 27.5 yr"},{v:"nonresidential",l:"Non-residential — 39 yr"}].map(o=>(
              <button key={o.v} onClick={()=>s("propType",o.v)} style={{padding:"9px 8px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",fontSize:11,textAlign:"center",transition:"all 0.15s",background:inp.propType===o.v?"rgba(200,169,110,0.12)":T.bgCard,border:"1px solid "+(inp.propType===o.v?T.gold:T.border),color:inp.propType===o.v?T.gold:T.textMid}}>{o.l}</button>
            ))}
          </div>
        </Fld>
        <div style={{padding:"12px 14px",background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,marginBottom:10}}>
          ...
          {inp.propType==="residential"&&<Fld label={"Residential structure — 27.5 yr"} hint={"Leave empty = all remaining basis"}><Inp value={inp.alloc27} onChange={v=>s("alloc27",v)} placeholder={fmt(Math.max(0,buildingBasis-parse(inp.alloc15)-parse(inp.alloc7)-parse(inp.alloc5)))} prefix="$"/></Fld>}
          {inp.propType==="nonresidential"&&<Fld label={"Non-residential — 39 yr"} hint={"Leave empty = all remaining basis"}><Inp value={inp.alloc39} onChange={v=>s("alloc39",v)} placeholder={fmt(Math.max(0,buildingBasis-parse(inp.alloc15)-parse(inp.alloc7)-parse(inp.alloc5)))} prefix="$"/></Fld>}
          <Fld label={"Land improvements — 15 yr (fencing, parking, landscaping)"} optional optLbl="optional"><Inp value={inp.alloc15} onChange={v=>s("alloc15",v)} placeholder="0" prefix="$"/></Fld>
          <Fld label={"Furniture/equipment — 7 yr"} optional optLbl="optional"><Inp value={inp.alloc7} onChange={v=>s("alloc7",v)} placeholder="0" prefix="$"/></Fld>
          <Fld label={"Appliances/carpet/fixtures — 5 yr"} optional optLbl="optional"><Inp value={inp.alloc5} onChange={v=>s("alloc5",v)} placeholder="0" prefix="$"/></Fld>
          {unallocated>0&&<div style={{padding:"6px 10px",background:"rgba(200,169,110,0.06)",border:"1px solid "+T.goldDim,borderRadius:5,fontSize:11,color:T.goldDim}}>Unallocated: <strong>{fmt(unallocated)}</strong> → auto-assigned to main class</div>}
          {allocTotal>buildingBasis&&<IBox tone="red">Total exceeds building basis: {fmt(buildingBasis)}</IBox>}
        </div>
        <div style={{padding:"12px 14px",background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,marginBottom:10}}>
          <Toggle value={inp.bonusDepreciation} onChange={v=>s("bonusDepreciation",v)} label={"Bonus Depreciation"} desc={"Personal property only (5/7/15 yr) — NOT real property structures"}/>
          {inp.bonusDepreciation&&<Fld label={"Rate (2024: 60% · 2025: 40% · 2026: 20%)"}><Inp value={inp.bonusPct} onChange={v=>s("bonusPct",v)} placeholder="60"/></Fld>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <Fld label={"Month placed in service"} hint={"Affects year 1 (mid-month convention)"}><Sel value={inp.monthPlaced} onChange={v=>s("monthPlaced",v)} options={months.map((m,i)=>({value:String(i+1),label:m}))}/></Fld>
        </div>
      ...
      {step===3&&res&&(<div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          <div style={{background:"linear-gradient(135deg,#0d1a08,#0a1205)",border:"1px solid #1a4020",borderRadius:10,padding:"16px 14px"}}>
            <div style={{fontSize:9,letterSpacing:"0.2em",textTransform:"uppercase",color:"#2a6030",marginBottom:6}}>{"First Year Depreciation"}</div>
            ...
            <div style={{fontSize:11,color:"#3a7040",marginTop:8}}>{"Annual (Year 2+): "}<strong>{fmt(res.totalAnnual)}</strong></div>
          </div>
          <div style={{background:"linear-gradient(135deg,#0a100a,#060a06)",border:"1px solid "+T.goldDim,borderRadius:10,padding:"16px 14px"}}>
            <div style={{fontSize:9,letterSpacing:"0.2em",textTransform:"uppercase",color:T.goldDim,marginBottom:6}}>{"Deductible Right Now"}</div>
            <div style={{fontSize:"clamp(26px,5vw,38px)",color:T.gold,fontWeight:300,lineHeight:1}}>{fmt(res.currentYearDeductible)}</div>
            <div style={{fontSize:10,color:T.goldDim,marginTop:4}}>{"Prepaid interest + property tax"}</div>
            {res.netLoanCosts>0&&<div style={{fontSize:10,color:T.textDim,marginTop:4}}>{"+ Loan costs (amortized): "}{fmt(res.netLoanCosts)}</div>}
          </div>
        </div>

        <Card>
          <CardTitle>{"Complete Closing Statement Analysis"}</CardTitle>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginTop:8}}>
            <tbody>
              <tr><td colSpan={2} style={{padding:"6px 0 2px",fontSize:10,color:"#4a6fa8",fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase"}}>{"PROPERTY BASIS"}</td></tr>
              <SRow label={"Purchase price"} value={fmt(parse(res.inp.purchasePrice))}/>
              {res.basisCC>0&&<SRow label={"+ Capitalized closing costs (B)"} value={fmt(res.basisCC)} color="#4a8fc0" indent/>}
              {res.unpaidAssumed>0&&<SRow label={"+ Unpaid items assumed (G)"} value={fmt(res.unpaidAssumed)} color="#c07050" indent/>}
              <SRow label={"= Total property basis"} value={fmt(res.totalBasis)} bold color={T.gold}/>
              <SRow label={"− Land basis (not depreciable)"} value={"("+fmt(res.landBasis)+")"} color={T.red} indent/>
              <SRow label={"= Building basis (depreciable)"} value={fmt(res.buildingBasis)} bold color={T.gold}/>
              <Divrow/>
              {res.hasloan&&res.grossLoanCosts>0&&(<>
                <tr><td colSpan={2} style={{padding:"6px 0 2px",fontSize:10,color:"#9a7a60",fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase"}}>{"LOAN COSTS (amortize over loan term)"}</td></tr>
                <SRow label={"Gross loan costs (C)"} value={fmt(res.grossLoanCosts)}/>
                {res.lenderCreditAmt>0&&<SRow label={"− Lender credit"} value={"("+fmt(res.lenderCreditAmt)+")"} color={T.green} indent/>}
                <SRow label={"= Net loan costs to amortize"} value={fmt(res.netLoanCosts)} bold/>
                <Divrow/>
              </>)}
              {res.currentYearDeductible>0&&(<>
                <tr><td colSpan={2} style={{padding:"6px 0 2px",fontSize:10,color:T.green,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase"}}>{"DEDUCTIBLE THIS YEAR"}</td></tr>
                {parse(res.inp.prepaidInterest)>0&&<SRow label={"Prepaid mortgage interest"} value={fmt(parse(res.inp.prepaidInterest))} color={T.green}/>}
                {parse(res.inp.prepaidTax)>0&&<SRow label={"Prepaid property tax"} value={fmt(parse(res.inp.prepaidTax))} color={T.green}/>}
                {parse(res.inp.proratedTaxCredit)>0&&<SRow label={"Prorated property tax credit"} value={fmt(parse(res.inp.proratedTaxCredit))} color={T.green}/>}
                <SRow label={"= Total deductible this year"} value={fmt(res.currentYearDeductible)} bold color={T.green}/>
                <Divrow/>
              </>)}
              {res.escrowTotal>0&&(<>
                <tr><td colSpan={2} style={{padding:"6px 0 2px",fontSize:10,color:"#7a6a9a",fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase"}}>{"ESCROW DEPOSITS (deduct when paid out)"}</td></tr>
                <SRow label={"Initial escrow deposits (E)"} value={fmt(res.escrowTotal)} color="#7a6a9a"/>
                <Divrow/>
              </>)}
              <tr><td colSpan={2} style={{padding:"6px 0 2px",fontSize:10,color:"#2a6030",fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase"}}>{"CASH-TO-CLOSE RECONCILIATION"}</td></tr>
              <SRow label={"Total charges to buyer"} value={fmt(res.totalCharges)}/>
              {parse(res.inp.sellerCredit)>0&&<SRow label={"− Seller credit (F)"} value={"("+fmt(parse(res.inp.sellerCredit))+")"} color={T.green} indent/>}
              {parse(res.inp.sellerPaidClosingCosts)>0&&<SRow label={"− Seller-paid costs (F)"} value={"("+fmt(parse(res.inp.sellerPaidClosingCosts))+")"} color={T.green} indent/>}
              {res.lenderCreditAmt>0&&<SRow label={"− Lender credit (C)"} value={"("+fmt(res.lenderCreditAmt)+")"} color={T.green} indent/>}
              {parse(res.inp.proratedTaxCredit)>0&&<SRow label={"− Prorated tax (F)"} value={"("+fmt(parse(res.inp.proratedTaxCredit))+")"} color={T.green} indent/>}
              {parse(res.inp.proratedRentCredit)>0&&<SRow label={"− Prorated rent (F)"} value={"("+fmt(parse(res.inp.proratedRentCredit))+")"} color={T.green} indent/>}
              {parse(res.inp.otherCredit)>0&&<SRow label={"− Other credits (F)"} value={"("+fmt(parse(res.inp.otherCredit))+")"} color={T.green} indent/>}
              {res.hasloan&&parse(res.inp.loanAmount)>0&&<SRow label={"− Loan amount (H)"} value={"("+fmt(parse(res.inp.loanAmount))+")"} color={T.green} indent/>}
              {parse(res.inp.downPayment)>0&&<SRow label={"− Down payment / deposit (H)"} value={"("+fmt(parse(res.inp.downPayment))+")"} color={T.green} indent/>}
              <SRow label={"= Estimated cash to close"} value={fmt(res.calcCashToClose)} bold color={T.gold}/>
            </tbody>
          </table>
          <div style={{marginTop:10,padding:"8px 12px",background:"rgba(74,144,96,0.08)",border:"1px solid #1a4020",borderRadius:6,fontSize:11,color:"#3a7050"}}>
            {"✓ If this matches your closing statement cash-to-close, all line items are correctly captured."}
          </div>
        </Card>

        <Card>
          <CardTitle>{"Depreciation Schedule by Asset Class"}</CardTitle>
          <div style={{overflowX:"auto",marginTop:8}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:480}}>
              <thead>
                <tr style={{borderBottom:"1px solid "+T.border}}>
                  {["Asset Class","Life","Basis","Bonus","Year 1","Annual","Recapture"].map((h,i)=>(
                    <th key={i} style={{textAlign:"left",padding:"5px 4px 5px 0",color:T.textDim,fontWeight:400,fontSize:8,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {res.classResults.map((c,i)=>(
                  <tr key={i} style={{borderBottom:"1px solid "+T.border}}>
                    <td style={{padding:"8px 4px 8px 0",color:c.color,fontWeight:500,fontSize:11}}>{c.label}</td>
          ...
          {res.classResults.some(c=>c.life<27.5)&&<IBox tone="green">{"✓ Cost Segregation: Accelerated depreciation for shorter-lived components — front-loads deductions instead of spreading over 27.5 years."}</IBox>}
        </Card>

        <Card>
          <CardTitle>{"⚠ Depreciation Recapture When You Sell"}</CardTitle>
          <p style={{fontSize:12,color:T.textMid,lineHeight:1.7,marginTop:6}}>{"Section 1250: All real property depreciation is recaptured at a flat 25% rate when you sell — regardless of your tax bracket."}</p>
            ...
            {[5,10,20].map(yr=>{
              ...
              return(
                <div key={yr} style={{background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,padding:"12px 10px",textAlign:"center"}}>
                  <div style={{fontSize:10,color:T.textDim,marginBottom:4}}>{"After "}{yr}{" yrs"}</div>
                  <div style={{fontSize:11,color:T.textMid,marginBottom:2}}>{"Depr.: "}{fmt(td)}</div>
                  <div style={{fontSize:15,color:T.red,fontWeight:600}}>{"@25%: "}{fmt(Math.round(td*0.25))}</div>
                </div>
              );
            })}
          </div>
          <IBox tone="amber">{"💡 A 1031 exchange can defer all recapture tax. See our 1031 calculator."}</IBox>
        </Card>

        <p style={{fontSize:10,color:T.textDim,textAlign:"center",lineHeight:1.6,marginTop:10}}>{"Estimates only — not tax advice. MACRS GDS straight-line. Consult a CPA before claiming."}</p>
