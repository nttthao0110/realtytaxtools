      ...
      {step===0&&(<div>
        ...
        {hasloan&&<SB color="#5a3a6a" tc="#c090d8" title="C — HUD-1 Expenses — Loan Cost Basis (amortize over loan term)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            ...
            <MF label={"Appraisal (Required by Lender)"} hint="On statement only — not POC" value={inp.appraisalLender} onChange={v=>s("appraisalLender",v)}/>
        ...
        <SB color="#1a4a3a" tc="#70d0a0" title="D — HUD-1 Expenses Currently Deductible (deduct in year of closing)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            <MF label={"Property Taxes Paid at Closing ← DEDUCTIBLE"} hint="Schedule E line 16" value={inp.propertyTaxClosing} onChange={v=>s("propertyTaxClosing",v)}/>
...
function CapGainsCalc({lang:L="en"}){
  const [inp,setInp]=useState({
    ...
    fs:"single",agi:"",primary:false,yearsOwned:"",yearsLived:""
  });
  const [res,setRes]=useState(null);
  const s=(k,v)=>setInp(p=>({...p,[k]:v}));
  const FS=[
  ...
  function doCalc(){
    ...
    // Must own AND live in home as primary residence for 2+ of last 5 years
    const yearsOwned=parse(inp.yearsOwned), yearsLived=parse(inp.yearsLived);
    const qualifies121=inp.primary && yearsOwned>=2 && yearsLived>=2;
    let exclusion=0;
    if(qualifies121){
  ...
  return(
    <div>
      ...
      <div style={{margin:"8px 0"}}>
        ...
        {inp.primary&&(
          <div style={{marginTop:8,padding:"12px 14px",background:T.bgCard2,border:"1px solid "+T.goldDim+"40",borderRadius:8}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <Fld label={"Years you OWNED the home"} hint={"Must be 2+ of last 5 years"}>
                <Inp value={inp.yearsOwned} onChange={v=>s("yearsOwned",v)} placeholder="0"/>
              </Fld>
              <Fld label={"Years you LIVED there as primary home"} hint={"Must be 2+ of last 5 years — can overlap with ownership"}>
                <Inp value={inp.yearsLived} onChange={v=>s("yearsLived",v)} placeholder="0"/>
              </Fld>
            </div>
            {(()=>{
              const owned=parse(inp.yearsOwned), lived=parse(inp.yearsLived);
              const qualifies=owned>=2&&lived>=2;
              const excl=inp.fs==="mfj"?500000:250000;
              if(!owned&&!lived) return null;
              return(
                <div style={{marginTop:8,padding:"8px 12px",borderRadius:6,
                  ...
                  {qualifies
                    ?<><span style={{color:T.green,fontWeight:600,fontSize:12}}>{"✅ Qualifies for Section 121 exclusion"}</span>
                      <span style={{color:T.textMid,fontSize:11}}>{" — up to "+fmt(excl)+" of gain excluded from tax"}</span></>
                    :<><span style={{color:T.red,fontWeight:600,fontSize:12}}>{"❌ Does not qualify"}</span>
                      <span style={{color:T.textMid,fontSize:11}}>{" — need 2+ years owned AND 2+ years lived as primary home in last 5 years"}</span></>
                  }
                </div>
              );
            })()}
      ...
      <div style={{display:"flex",gap:8,marginTop:8}}>
        ...
        <OutlineBtn onClick={()=>{setInp({saleDate:"",purchaseDate:"",salePrice:"",basis:"",improvements:"",depreciation:"",expenses:"",fs:"single",agi:"",primary:false,yearsOwned:"",yearsLived:""});setRes(null);}}>{"Reset"}</OutlineBtn>
      </div>
