...
const LANG={
  en:{
    code:"EN",
    nav:{home:"Home",calculators:"Calculators",blog:"Blog",pricing:"Pricing",shop:"Shop",about:"About"},
    hero:{
...
function CostSegCalc({lang:L="en"}){
  const [inp,setInp]=useState({
    ...
    loanAmount:"",interestRate:"",loanTerm:"30",
  });
  const [res,setRes]=useState(null);
  const s=(k,v)=>setInp(p=>({...p,[k]:v}));

  const buildingBasis=Math.max(0,parse(inp.purchasePrice)*parse(inp.buildingPct)/100);
  // Typical cost seg allocation: 15% appliances/5yr, 10% 15yr, 75% structure
  const seg5=Math.round(buildingBasis*0.15);
  const seg15=Math.round(buildingBasis*0.10);
  const struct=buildingBasis-seg5-seg15;
  const bonusPct=0.40; // 2025: 40%
  // Year 1 depreciation WITH cost seg
  const bonus5=Math.round(seg5*bonusPct);
  const bonus15=Math.round(seg15*bonusPct);
  const yr1_5=bonus5+Math.round((seg5-bonus5)*2/5*0.5);
  const yr1_15=bonus15+Math.round((seg15-bonus15)*1.5/15*0.5);
  const yr1_struct=struct>0?Math.round(struct/27.5*11.5/12):0;
  const totalWithCostSeg=yr1_5+yr1_15+yr1_struct;
  // Year 1 WITHOUT cost seg (straight line only)
  const totalWithout=buildingBasis>0?Math.round(buildingBasis/27.5*11.5/12):0;
  const extraDepr=totalWithCostSeg-totalWithout;
  const bracket=parse(inp.taxBracket)/100;
  const taxSaving=Math.round(extraDepr*bracket);
  ...
  return(
    <div>
      <div style={{marginBottom:16}}>
        ...
        <p style={{margin:0,fontSize:12,color:T.textDim}}>{"See how much extra depreciation you could unlock with a cost seg study — and whether it's worth the $3,000–8,000 study cost."}</p>
      </div>

      <div style={{padding:"12px 14px",background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,marginBottom:10}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <Fld label="Purchase price" hint="Total property cost"><Inp value={inp.purchasePrice} onChange={v=>s("purchasePrice",v)} placeholder="300000" prefix="$"/></Fld>
          <Fld label="Building value %" hint="Exclude land (typically 75–85%)"><Inp value={inp.buildingPct} onChange={v=>s("buildingPct",v)} placeholder="80"/></Fld>
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
            <div style={{padding:"12px",background:"rgba(40,40,60,0.5)",border:"1px solid "+T.border,borderRadius:8}}>
              ...
              <div style={{fontSize:9,color:T.textDim,marginBottom:2}}>{"Annual (all years)"}</div>
              <div style={{fontSize:14,color:T.textMid}}>{fmt(annualStandard)}</div>
              <div style={{fontSize:9,color:T.textDim,marginTop:8}}>{"27.5yr straight-line only"}</div>
            </div>
            <div style={{padding:"12px",background:"linear-gradient(135deg,#0a1a0e,#080e0a)",border:"1px solid #1a4020",borderRadius:8}}>
              ...
              <div style={{fontSize:9,color:T.textDim,marginBottom:4}}>{"Year 1 Depreciation"}</div>
              <div style={{fontSize:24,color:T.green,fontWeight:300,marginBottom:4}}>{fmt(totalWithCostSeg)}</div>
              <div style={{fontSize:9,color:T.textDim,marginBottom:2}}>{"Annual (Yr 2+)"}</div>
              <div style={{fontSize:14,color:T.green}}>{fmt(annualStandard)}</div>
              <div style={{fontSize:9,color:"#3a7040",marginTop:8}}>{"Bonus + 5/15yr components"}</div>
            </div>
          </div>

          {/* The money numbers */}
          <div style={{background:"linear-gradient(135deg,#0d1a08,#0a1205)",border:"1px solid #1a4020",borderRadius:10,padding:"14px 16px",marginBottom:12}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div>
                <div style={{fontSize:9,color:T.textDim,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>{"Extra Deduction"}</div>
                <div style={{fontSize:26,color:T.gold,fontWeight:300}}>{fmt(extraDepr)}</div>
              ...
              <div>
                ...
                <div style={{fontSize:26,color:T.green,fontWeight:300}}>{fmt(taxSaving)}</div>
            ...
            {taxSaving>0&&(
              <div style={{marginTop:12,padding:"8px 12px",background:"rgba(74,144,96,0.1)",borderRadius:6,fontSize:11}}>
                <span style={{color:T.textMid}}>{"Cost seg study typically costs $3,000–8,000. At your bracket, "}</span>
                <strong style={{color:taxSaving>5000?T.green:T.gold}}>
                  {taxSaving>8000?"this almost certainly pays for itself in Year 1.":
                   taxSaving>4000?"this likely pays for itself in Year 1.":
                   "consider whether the Year 1 savings justify the study cost."}
                </strong>
              </div>
            )}
          </div>
          ...
          <div style={{padding:"10px 14px",background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,marginBottom:12}}>
            ...
            <p style={{fontSize:9,color:T.textDim,margin:"0 0 8px",fontStyle:"italic"}}>{"Typical allocation — your actual study may differ. Based on IRS field audit guidelines."}</p>
            {[
              {label:"Structure (27.5yr)",basis:struct,yr1:yr1_struct,color:T.textMid},
              {label:"Land improvements (15yr) — fencing, driveway",basis:seg15,yr1:yr1_15,color:"#9a7a60"},
              {label:"Personal property (5yr) — appliances, carpet",basis:seg5,yr1:yr1_5,color:T.gold},
            ].map((c,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",borderBottom:"1px solid "+T.border+"40",fontSize:10}}>
                <span style={{color:c.color}}>{c.label}</span>
                <span style={{color:T.textMid}}>{fmt(c.basis)+" basis · "+fmt(c.yr1)+" Yr1"}</span>
              </div>
            ))}
          </div>

          <IBox>{"This is an estimate based on typical allocations. A real cost segregation study requires an engineering analysis. For properties under $500k, consider a mini study ($1,000–2,500). Always consult a CPA before claiming."}</IBox>
          ...
          <div style={{marginTop:12,padding:"12px 16px",background:"linear-gradient(135deg,rgba(200,169,110,0.08),rgba(200,169,110,0.04))",border:"1px solid "+T.goldDim+"50",borderRadius:10,textAlign:"center"}}>
            <p style={{fontSize:12,color:T.gold,fontWeight:700,margin:"0 0 4px"}}>{"Want the full cost seg worksheet?"}</p>
            <p style={{fontSize:11,color:T.textDim,margin:"0 0 10px"}}>{"The Real Estate Tax Workbook includes a cost seg tracker, depreciation schedule, and basis calculator in one Excel file."}</p>
