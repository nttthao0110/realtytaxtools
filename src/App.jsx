          ...
          {step===0&&(
            <div>
              ...
              <DeprecSection title="B — HUD-1 Expenses Added to Property Cost Basis" color="#2a3a6a" tc="#90b0f0">
                ...
                <DeprecRow label="Title Insurance — Lender's Policy  (→ Loan Cost, not basis)" hint="Lender's title policy is a loan acquisition cost — enter in Loan Costs section below, not here. Owner's title policy adds to basis." hint="HUD-1 line 1101" value={inp.titleInsuranceLender} onChange={v=>s("titleInsuranceLender",v)}/>
    ...
    <div style={{maxWidth:800,margin:"40px auto 0",padding:"0 20px 40px"}}>
      ...
      {[
        ...
        {q:"Does cost segregation increase my depreciation?",
         a:"Yes. Cost segregation reclassifies components into shorter depreciation categories — 5-year, 7-year, and 15-year — instead of the standard 27.5-year schedule. Combined with bonus depreciation (40% under TCJA in 2025, potentially 100% if OBBBA is enacted for property placed in service after 1/19/25), cost segregation can generate significant Year 1 deductions on properties valued at $300K or more."},
      ].map((f,i)=>(
        <div key={i} style={{marginBottom:20,padding:"16px",background:T.bgCard,borderRadius:8,border:"1px solid "+T.border}}>
          <p style={{fontSize:14,fontWeight:600,color:T.gold,margin:"0 0 8px"}}>{f.q}</p>
          <p style={{fontSize:13,color:T.textMid,lineHeight:1.7,margin:0}}>{f.a}</p>
...
function CostSegSEOBlock(){
  return(
    <div style={{maxWidth:800,margin:"40px auto 0",padding:"0 20px 40px"}}>
      ...
      <p style={{fontSize:14,color:T.textMid,lineHeight:1.7,marginBottom:14}}>
        {"Bonus depreciation allows investors to immediately deduct a percentage of qualifying 5-, 7-, and 15-year property placed in service during the year. Under TCJA, the 2025 rate is 40%. The One Big Beautiful Bill Act (OBBBA), if enacted, would restore 100% bonus depreciation for qualifying property placed in service after January 19, 2025 — consult your CPA for current status. Cost segregation identifies the assets eligible for bonus depreciation."}
      </p>
  ...
  costseg: {
    ...
    faq: {
      ...
      "mainEntity":[
        ...
        {"@type":"Question","name":"What is bonus depreciation and how does it work in 2025?","acceptedAnswer":{"@type":"Answer","text":"Bonus depreciation (Section 168(k)) allows qualifying 5-, 7-, and 15-year property to be deducted at an accelerated rate in Year 1. Under TCJA, the rate is 40% in 2025. The One Big Beautiful Bill Act (OBBBA), if enacted, would restore 100% bonus for qualifying property placed in service after January 19, 2025. Without bonus depreciation, cost segregation assets are still depreciated faster than 27.5 years."}},
...
export default function App(){
  ...
  // ── Read URL params on first load so Google can crawl ?calc=X directly ──
  ...
  function getInitialState(){
    ...
    return {
      page: calc&&VALID_CALCS.includes(calc) ? "calculators" : (pg||"home"),
      calc: calc&&VALID_CALCS.includes(calc) ? calc : "underpayment",
    };
  }
  const _init=getInitialState();
  const [page,setPage]=useState(_init.page);
  const [activeCalc,setActiveCalc]=useState(_init.calc);
  ...
  useEffect(()=>{
    ...
    const descs={
      depreciation:'Free rental property depreciation calculator built by a CPA. Calculate your depreciable basis, land allocation, and annual Schedule E deduction in seconds.',
      costseg:'Free cost segregation estimator. See how much you could save in Year 1 with accelerated depreciation on your rental property. Built by a licensed CPA.',
      exchange1031:'Free 1031 exchange calculator. See exactly how much capital gains tax and depreciation recapture a like-kind exchange defers. CPA-built, free to use.',
      capitalgains:'Free capital gains tax calculator for rental property sales. Includes depreciation recapture, NIIT, federal and state rates. Built by a licensed CPA.',
      str:'Free short-term rental tax calculator. Estimate your STR income tax, self-employment tax, and deductible expenses. Built by a real estate CPA.',
      underpayment:'Free IRS underpayment penalty calculator. Estimate your Q1-Q4 safe harbor amounts and avoid surprise penalties. Updated for 2025 IRS rates.',
      mortgage:'Free rental property mortgage calculator. See monthly payment, amortization, cash-on-cash return, and cap rate. Built by a real estate CPA.',
    };
    let metaDesc=document.querySelector('meta[name="description"]');
    if(!metaDesc){metaDesc=document.createElement('meta');metaDesc.name='description';document.head.appendChild(metaDesc);}
    if(descs[activeCalc]) metaDesc.content=descs[activeCalc];

    // Update canonical URL so Google indexes each calculator separately
    let canonical=document.querySelector('link[rel="canonical"]');
    if(!canonical){canonical=document.createElement('link');canonical.rel='canonical';document.head.appendChild(canonical);}
    canonical.href=`https://realtytaxtools.com/?calc=${activeCalc}`;

    // Update URL bar without triggering a page reload
    const newUrl=`${window.location.pathname}?calc=${activeCalc}`;
    window.history.replaceState({calc:activeCalc},'',newUrl);
  ...
  function nav(p){
    ...
    // Clear calc param when navigating away from calculators
    if(p!=="calculators"){
