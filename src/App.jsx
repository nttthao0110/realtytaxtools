...
function UnderpaymentCalc({lang}){
  ...
  const [inp,setInp]=useState({
    ...
    // mixed
    w2mixed:"", w2whMixed:"", selfMixed:"", selfExpMixed:"", rentalMixed:"",
    // shared
    priorTax:"", priorAGI:"",
    unevenIncome:null,
  ...
  // ── Income & tax calculation per type ───────────────────────────────────────
  function calcIncomeAndTax(){
    const it=inp.incomeType;
    let netIncome=0, wh=0, seTax=0;

    if(it==="self"){
      netIncome=Math.max(0,parse(inp.gross)-parse(inp.expenses));
      wh=0;
      // SE tax: 92.35% of net × 15.3%, then deduct half SE from income
      seTax=Math.round(netIncome*0.9235*0.153);
      const seDeduct=seTax/2;
      const taxable=Math.max(0,netIncome-seDeduct-(STD_DED[inp.fs]||14600));
      const incomeTax=estTax(taxable+(STD_DED[inp.fs]||14600),inp.fs);
      return{netIncome,wh,seTax,totalTax:seTax+incomeTax,incomeTax};
    }

    if(it==="both"){
      const w2=parse(inp.w2income);
      wh=parse(inp.w2wh);
      const side=Math.max(0,parse(inp.sideIncome)-parse(inp.sideExpenses));
      seTax=Math.round(side*0.9235*0.153);
      const seDeduct=seTax/2;
      const totalGross=w2+side-seDeduct;
      const incomeTax=estTax(Math.max(0,totalGross),inp.fs);
      netIncome=w2+side;
      return{netIncome,wh,seTax,totalTax:seTax+incomeTax,incomeTax};
    }

    if(it==="landlord"){
      netIncome=Math.max(0,parse(inp.rentalIncome)-parse(inp.rentalExpenses));
      wh=0;
      // Rental income = ordinary income, no SE tax
      const incomeTax=estTax(Math.max(0,netIncome),inp.fs);
      return{netIncome,wh,seTax:0,totalTax:incomeTax,incomeTax};
    }

    if(it==="mixed"){
      const w2=parse(inp.w2mixed);
      wh=parse(inp.w2whMixed);
      const self=Math.max(0,parse(inp.selfMixed)-parse(inp.selfExpMixed));
      const rental=parse(inp.rentalMixed);
      seTax=Math.round(self*0.9235*0.153);
      const seDeduct=seTax/2;
      const totalGross=w2+self+rental-seDeduct;
      const incomeTax=estTax(Math.max(0,totalGross),inp.fs);
      netIncome=w2+self+rental;
      return{netIncome,wh,seTax,totalTax:seTax+incomeTax,incomeTax};
    }
    return{netIncome:0,wh:0,seTax:0,totalTax:0,incomeTax:0};
  }

  const {netIncome,wh:derivedWH,seTax,totalTax,incomeTax}=calcIncomeAndTax();
  ...
  const currentSafeHarbor=totalTax*0.90;
  const safeHarborAmount=priorSafeHarbor>0
    ? Math.min(currentSafeHarbor>0?currentSafeHarbor:Infinity, priorSafeHarbor)
    : currentSafeHarbor;

  // ── Translations ─────────────────────────────────────────────────────────────
  const TX={
    title:L==="vi"?"Máy Tính Thuế Ước Tính Hàng Quý":L==="es"?"Calculadora de Impuesto Trimestral":"Quarterly Tax Payment Calculator",
    sub:L==="vi"?"Tính chính xác số tiền cần nộp mỗi quý để tránh bị IRS phạt":L==="es"?"Calcule exactamente cuánto pagar cada trimestre para evitar multas":"Calculate exactly how much to pay each quarter to avoid IRS penalties",
    ...
    incomeTypes:[
      {v:"self",  icon:"💼", l:L==="vi"?"Tự kinh doanh / 1099":L==="es"?"Autónomo / 1099":"Self-employed / 1099",
       desc:L==="vi"?"Freelancer, nail tech, môi giới, chủ doanh nghiệp":L==="es"?"Freelancer, contratista, dueño de negocio":"Freelancer, contractor, business owner"},
      {v:"both",  icon:"🏢", l:L==="vi"?"W-2 + Thu nhập phụ":L==="es"?"W-2 + Ingresos adicionales":"W-2 job + side income",
       desc:L==="vi"?"Có lương + thu nhập 1099 hoặc kinh doanh thêm":L==="es"?"Tiene salario más ingresos adicionales 1099":"Have a salary plus 1099 or freelance income"},
      {v:"landlord",icon:"🏠", l:L==="vi"?"Chủ nhà cho thuê":L==="es"?"Propietario de alquiler":"Rental property owner",
       desc:L==="vi"?"Thu nhập từ cho thuê bất động sản":L==="es"?"Ingresos de propiedades en alquiler":"Income from rental properties"},
      {v:"mixed",  icon:"📊", l:L==="vi"?"Kết hợp nhiều loại":L==="es"?"Múltiples fuentes":"Multiple income types",
       desc:L==="vi"?"W-2 + tự kinh doanh + cho thuê":L==="es"?"W-2 + autónomo + alquiler":"W-2 + self-employment + rental"},
    ],
    ...
    priorTaxH:L==="vi"?"Mẫu 1040 năm ngoái, Dòng 24":L==="es"?"Formulario 1040 del año pasado, Línea 24":"Last year's Form 1040, Line 24",
    ...
    unevenQ:L==="vi"?"Thu nhập của bạn có đều đặn mỗi quý không?":L==="es"?"¿Sus ingresos son similares cada trimestre?":"Is your income roughly the same each quarter?",
    ...
    disc:L==="vi"?"Chỉ ước tính — không phải tư vấn thuế. Dựa trên IRS Ấn phẩm 505.":L==="es"?"Solo estimaciones — no es asesoramiento fiscal. Basado en Publicación 505.":"Estimates only — not tax advice. Based on IRS Publication 505 and Form 2210.",
  };

  const steps=[TX.step1title,TX.step2title,TX.step3title,TX.step4title];

  // Step 2 validation depends on income type
  const canGo2=
    inp.incomeType==="self"    ? parse(inp.gross)>0 :
    inp.incomeType==="both"    ? parse(inp.w2income)>0||parse(inp.sideIncome)>0 :
    inp.incomeType==="landlord"? parse(inp.rentalIncome)>0 :
    inp.incomeType==="mixed"   ? parse(inp.w2mixed)>0||parse(inp.selfMixed)>0||parse(inp.rentalMixed)>0 : false;
  ...
  function doCalc(){
    ...
    if(inp.unevenIncome===false){
      const perQ=safeHarborAmount/4;
      qRequired=[perQ,perQ,perQ,perQ];
    } else {
      ...
      const cumRequired=cumInc.map((ci,i)=>{
        const annualized=ci*AI_FACTORS[i];
        const annSE=Math.round(annualized*0.9235*0.153);
        const annSeDeduct=annSE/2;
        const annTaxable=Math.max(0,annualized-annSeDeduct);
        const annTax=(inp.incomeType==="landlord")?estTax(annualized,inp.fs):estTax(annTaxable,inp.fs)+annSE;
        const aiReq=annTax*AI_PCT[i];
        const priorReq=safeHarborAmount*(i+1)/4;
        return Math.min(aiReq,priorReq);
      });
      qRequired=cumRequired.map((cr,i)=>Math.max(0,cr-(i>0?cumRequired[i-1]:0)));
    }

    const perQWH=wh/4;
    let totalPenalty=0;
    const quarters=QQ.map((q,i)=>{
      const required=qRequired[i];
      const paid=qPaid[i];
      const whCredit=perQWH;
      const totalForQ=paid+whCredit;
      const shortfall=Math.max(0,required-totalForQ);
      const penalty=q.past?Math.round(shortfall*(IRS_RATE/365)*q.days):0;
      totalPenalty+=penalty;
      return{...q,required,paid,whCredit,totalForQ,shortfall,penalty};
    });
    ...
    setRes({
      netIncome,wh,seTax,totalTax,incomeTax,
      safeHarborAmount,shPct,priorSafeHarbor,currentSafeHarbor,
      totalPaid,totalPenalty,quarters,nextQ,catchUp,
      unevenIncome:inp.unevenIncome,incomeType:inp.incomeType,
    });
    setStep(4);
  }
  ...
  return(
    <div style={{maxWidth:600,margin:"0 auto"}}>
      ...
      {step===0&&(<Card>
        ...
        <CardSub>{L==="vi"?"Chọn loại thu nhập — mỗi loại có cách tính thuế khác nhau":L==="es"?"Seleccione su tipo — cada uno calcula impuestos de forma diferente":"Select your type — each calculates taxes differently"}</CardSub>
        <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
          {TX.incomeTypes.map(o=>(
            <button key={o.v} onClick={()=>s("incomeType",o.v)}
              style={{padding:"12px 16px",borderRadius:8,cursor:"pointer",fontFamily:"inherit",textAlign:"left",transition:"all 0.15s",
                background:inp.incomeType===o.v?"rgba(200,169,110,0.12)":T.bgCard,
                border:`1px solid ${inp.incomeType===o.v?T.gold:T.border}`}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontSize:18}}>{o.icon}</span>
                <div>
                  <div style={{fontSize:13,color:inp.incomeType===o.v?T.gold:T.text,fontWeight:500,marginBottom:2}}>{o.l}</div>
      ...
      {/* STEP 1 — Income (different per type) */}
      {step===1&&(<Card>
        <CardTitle>{TX.step2title}</CardTitle>

        {/* SELF-EMPLOYED */}
        {inp.incomeType==="self"&&(<>
          <CardSub>{L==="vi"?"SE tax (15.3%) sẽ được tính tự động":L==="es"?"SE tax (15.3%) se calculará automáticamente":"SE tax (15.3%) will be calculated automatically"}</CardSub>
          <Fld label={L==="vi"?"Tổng thu nhập kinh doanh":L==="es"?"Ingresos brutos del negocio":"Gross business income"}
               hint={L==="vi"?"Tổng doanh thu trước chi phí":L==="es"?"Ingresos totales antes de gastos":"Total revenue before expenses"}>
            <Inp value={inp.gross} onChange={v=>s("gross",v)} placeholder="0" prefix="$"/>
          </Fld>
          <Fld label={L==="vi"?"Chi phí kinh doanh":L==="es"?"Gastos del negocio":"Business expenses"}
               hint={L==="vi"?"Marketing, xe hơi, văn phòng, v.v.":L==="es"?"Marketing, vehículo, oficina, etc.":"Marketing, vehicle, office, etc."} optional optLbl={TX.opt}>
            <Inp value={inp.expenses} onChange={v=>s("expenses",v)} placeholder="0" prefix="$"/>
          </Fld>
          {parse(inp.gross)>0&&(
            <IBox tone="green">
              {L==="vi"?"Thu nhập ròng":L==="es"?"Ingreso neto":"Net income"}: <strong>{fmt(netIncome)}</strong>
              {" · "}SE tax: <strong>{fmt(seTax)}</strong>
              {" · "}{L==="vi"?"Thuế thu nhập":L==="es"?"Impuesto renta":"Income tax"}: <strong>{fmt(incomeTax)}</strong>
              {" · "}{L==="vi"?"Tổng thuế ước tính":L==="es"?"Total estimado":"Total est. tax"}: <strong style={{color:T.gold}}>{fmt(totalTax)}</strong>
            </IBox>
          )}
        </>)}
        ...
        {/* W-2 + SIDE */}
        {inp.incomeType==="both"&&(<>
          <CardSub>{L==="vi"?"SE tax áp dụng cho thu nhập phụ":L==="es"?"SE tax aplica a los ingresos adicionales":"SE tax applies to your side income"}</CardSub>
          <div style={{padding:"12px 14px",background:T.bgCard2,border:`1px solid ${T.border}`,borderRadius:8,marginBottom:14}}>
            <p style={{fontSize:11,color:T.gold,margin:"0 0 10px",fontWeight:500,textTransform:"uppercase",letterSpacing:"0.06em"}}>
              {L==="vi"?"Lương W-2":L==="es"?"Salario W-2":"W-2 Salary"}
            </p>
            <Fld label={L==="vi"?"Thu nhập W-2 (lương gộp)":L==="es"?"Ingreso W-2 (salario bruto)":"W-2 income (gross salary)"}>
              <Inp value={inp.w2income} onChange={v=>s("w2income",v)} placeholder="0" prefix="$"/>
            </Fld>
            <Fld label={L==="vi"?"Thuế đã khấu trừ từ W-2":L==="es"?"Impuesto retenido del W-2":"Federal tax withheld from W-2"}
                 hint="W-2 Box 2">
              <Inp value={inp.w2wh} onChange={v=>s("w2wh",v)} placeholder="0" prefix="$"/>
            </Fld>
          </div>
          <div style={{padding:"12px 14px",background:T.bgCard2,border:`1px solid ${T.border}`,borderRadius:8,marginBottom:14}}>
            <p style={{fontSize:11,color:T.gold,margin:"0 0 10px",fontWeight:500,textTransform:"uppercase",letterSpacing:"0.06em"}}>
              {L==="vi"?"Thu nhập phụ / 1099":L==="es"?"Ingresos adicionales / 1099":"Side income / 1099"}
            </p>
            <Fld label={L==="vi"?"Thu nhập phụ gộp":L==="es"?"Ingresos adicionales brutos":"Gross side income"}>
              <Inp value={inp.sideIncome} onChange={v=>s("sideIncome",v)} placeholder="0" prefix="$"/>
            </Fld>
            <Fld label={L==="vi"?"Chi phí liên quan":L==="es"?"Gastos relacionados":"Related expenses"} optional optLbl={TX.opt}>
              <Inp value={inp.sideExpenses} onChange={v=>s("sideExpenses",v)} placeholder="0" prefix="$"/>
            </Fld>
          </div>
          {(parse(inp.w2income)>0||parse(inp.sideIncome)>0)&&(
            <IBox tone="green">
              {L==="vi"?"Tổng thu nhập":L==="es"?"Ingreso total":"Total income"}: <strong>{fmt(netIncome)}</strong>
              {seTax>0&&<> · SE tax: <strong>{fmt(seTax)}</strong></>}
              {" · "}{L==="vi"?"Tổng thuế ước tính":L==="es"?"Total estimado":"Total est. tax"}: <strong style={{color:T.gold}}>{fmt(totalTax)}</strong>
              {" · "}{L==="vi"?"Khấu trừ W-2":L==="es"?"Retención W-2":"W-2 withholding"}: <strong>{fmt(parse(inp.w2wh))}</strong>
            </IBox>
          )}
        </>)}
        ...
        {inp.incomeType==="landlord"&&(<>
          <CardSub>{L==="vi"?"Thu nhập cho thuê = thu nhập thường, không có SE tax":L==="es"?"Ingresos de alquiler = renta ordinaria, sin SE tax":"Rental income = ordinary income, no SE tax"}</CardSub>
          <Fld label={L==="vi"?"Tổng thu nhập cho thuê":L==="es"?"Ingresos totales de alquiler":"Total rental income"}
               hint={L==="vi"?"Tất cả bất động sản cộng lại":L==="es"?"Todas las propiedades combinadas":"All properties combined"}>
            <Inp value={inp.rentalIncome} onChange={v=>s("rentalIncome",v)} placeholder="0" prefix="$"/>
          </Fld>
          <Fld label={L==="vi"?"Tổng chi phí cho thuê":L==="es"?"Gastos totales de alquiler":"Total rental expenses"}
               hint={L==="vi"?"Thế chấp, sửa chữa, bảo hiểm, quản lý, khấu hao, v.v.":L==="es"?"Hipoteca, reparaciones, seguro, gestión, depreciación, etc.":"Mortgage interest, repairs, insurance, management, depreciation, etc."} optional optLbl={TX.opt}>
            <Inp value={inp.rentalExpenses} onChange={v=>s("rentalExpenses",v)} placeholder="0" prefix="$"/>
          </Fld>
          {parse(inp.rentalIncome)>0&&(
            <IBox tone="green">
              {L==="vi"?"Thu nhập ròng":L==="es"?"Ingreso neto":"Net rental income"}: <strong>{fmt(netIncome)}</strong>
              {" · "}{L==="vi"?"Không có SE tax":L==="es"?"Sin SE tax":"No SE tax"}
              {" · "}{L==="vi"?"Thuế thu nhập ước tính":L==="es"?"Impuesto estimado":"Est. income tax"}: <strong style={{color:T.gold}}>{fmt(totalTax)}</strong>
            </IBox>
          )}
        </>)}
        ...
        {inp.incomeType==="mixed"&&(<>
          <CardSub>{L==="vi"?"Nhập tất cả nguồn thu nhập":L==="es"?"Ingrese todas sus fuentes de ingresos":"Enter all your income sources"}</CardSub>
          <div style={{padding:"12px 14px",background:T.bgCard2,border:`1px solid ${T.border}`,borderRadius:8,marginBottom:10}}>
            <p style={{fontSize:11,color:T.gold,margin:"0 0 10px",fontWeight:500,textTransform:"uppercase",letterSpacing:"0.06em"}}>W-2</p>
          ...
          <div style={{padding:"12px 14px",background:T.bgCard2,border:`1px solid ${T.border}`,borderRadius:8,marginBottom:10}}>
            <p style={{fontSize:11,color:T.gold,margin:"0 0 10px",fontWeight:500,textTransform:"uppercase",letterSpacing:"0.06em"}}>{L==="vi"?"Tự kinh doanh":L==="es"?"Autónomo":"Self-employment"}</p>
            <Fld label={L==="vi"?"Thu nhập tự kinh doanh gộp":L==="es"?"Ingresos autónomo brutos":"Gross self-employment income"} optional optLbl={TX.opt}><Inp value={inp.selfMixed} onChange={v=>s("selfMixed",v)} placeholder="0" prefix="$"/></Fld>
            <Fld label={L==="vi"?"Chi phí kinh doanh":L==="es"?"Gastos del negocio":"Business expenses"} optional optLbl={TX.opt}><Inp value={inp.selfExpMixed} onChange={v=>s("selfExpMixed",v)} placeholder="0" prefix="$"/></Fld>
          </div>
          <div style={{padding:"12px 14px",background:T.bgCard2,border:`1px solid ${T.border}`,borderRadius:8,marginBottom:10}}>
            <p style={{fontSize:11,color:T.gold,margin:"0 0 10px",fontWeight:500,textTransform:"uppercase",letterSpacing:"0.06em"}}>{L==="vi"?"Thu nhập cho thuê":L==="es"?"Alquiler":"Rental"}</p>
          ...
          {netIncome>0&&(
            <IBox tone="green">
              {L==="vi"?"Tổng thu nhập":L==="es"?"Total":"Total"}: <strong>{fmt(netIncome)}</strong>
              {seTax>0&&<> · SE tax: <strong>{fmt(seTax)}</strong></>}
              {" · "}{L==="vi"?"Tổng thuế ước tính":L==="es"?"Total estimado":"Total est. tax"}: <strong style={{color:T.gold}}>{fmt(totalTax)}</strong>
              {parse(inp.w2whMixed)>0&&<> · {L==="vi"?"Khấu trừ":L==="es"?"Retención":"Withholding"}: <strong>{fmt(parse(inp.w2whMixed))}</strong></>}
            </IBox>
          )}
        </>)}

        <NavRow><Btn ghost onClick={()=>setStep(0)}>{TX.back}</Btn><Btn onClick={()=>setStep(2)} disabled={!canGo}>{TX.next}</Btn></NavRow>
      </Card>)}

      {/* STEP 2 — Prior year */}
      {step===2&&(<Card>
        <CardTitle>{TX.step3title}</CardTitle>
        <CardSub>{L==="vi"?"Dùng để tính ngưỡng an toàn (safe harbor)":L==="es"?"Para calcular el umbral de puerto seguro":"Used to calculate your safe harbor amount"}</CardSub>
        ...
        {parse(inp.priorAGI)>0&&parse(inp.priorTax)>0&&(
          <IBox tone={priorAGI>thr?undefined:"green"}>
            ...
            {totalTax>0&&<> · 90% {L==="vi"?"năm nay":L==="es"?"actual":"current yr"} = <strong>{fmt(currentSafeHarbor)}</strong> · {L==="vi"?"Dùng":L==="es"?"Usar":"Use"}: <strong style={{color:T.gold}}>{fmt(safeHarborAmount)}</strong></>}
          </IBox>
        )}
      ...
      {step===3&&(<Card>
        ...
        <CardSub>{L==="vi"?"Nhập những gì bạn đã nộp trực tiếp cho IRS":L==="es"?"Ingrese lo que ya pagó directamente al IRS":"Enter what you've paid directly to the IRS"}</CardSub>
            ...
            {[{v:false,l:TX.unevenYes},{v:true,l:TX.unevenNo}].map(o=>(
              <button key={String(o.v)} onClick={()=>s("unevenIncome",o.v)}
                style={{padding:"11px 8px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",fontSize:12,lineHeight:1.4,textAlign:"center",transition:"all 0.15s",
                  background:inp.unevenIncome===o.v?"rgba(200,169,110,0.12)":T.bgCard,
                  border:`1px solid ${inp.unevenIncome===o.v?T.gold:T.border}`,
                  color:inp.unevenIncome===o.v?T.gold:T.textMid}}>
                {o.l}
              </button>
            ))}
          </div>
        ...
        {inp.unevenIncome===true&&(
          <div style={{marginBottom:14,padding:"12px 14px",background:T.bgCard2,border:`1px solid ${T.border}`,borderRadius:8}}>
            <p style={{fontSize:11,color:T.textMid,margin:"0 0 10px"}}>{L==="vi"?"Thu nhập thực tế từng quý":L==="es"?"Ingresos reales por trimestre":"Actual income earned each quarter"}:</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {QQ.map((q,i)=>(
                <Fld key={i} label={`${q.l} · ${q.mo}`}><Inp value={inp[`q${i+1}inc`]} onChange={v=>s(`q${i+1}inc`,v)} placeholder="0" prefix="$"/></Fld>
              ))}
            </div>
          </div>
        )}
        {inp.unevenIncome!==null&&(<>
          <Divider/>
          <p style={{fontSize:11,color:T.textMid,margin:"0 0 10px",fontWeight:500}}>
            {L==="vi"?"Số tiền đã nộp trực tiếp IRS mỗi quý:":L==="es"?"Monto pagado directamente al IRS por trimestre:":"Amount paid directly to IRS each quarter:"}
          </p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {QQ.map((q,i)=>(
              <Fld key={i} label={`${q.l} · ${q.due}`}
                hint={q.past?(L==="vi"?"Đã qua hạn":L==="es"?"Ya venció":"Past due"):(L==="vi"?"Chưa đến hạn":L==="es"?"Próximo":"Upcoming")}>
                <Inp value={inp[`q${i+1}paid`]} onChange={v=>s(`q${i+1}paid`,v)} placeholder="0" prefix="$"/>
              </Fld>
            ))}
          </div>
        </>)}
      ...
      {step===4&&res&&(<div>
        ...
        {res.nextQ&&res.nextQ.shortfall>0?(
          <div style={{background:"linear-gradient(135deg,#0d1a08,#0a1505)",border:"1px solid #1a4020",borderRadius:12,padding:"24px 20px",textAlign:"center",marginBottom:14}}>
            <div style={{fontSize:9,letterSpacing:"0.3em",textTransform:"uppercase",color:"#2a6030",marginBottom:8}}>
              {L==="vi"?"Thanh Toán Tiếp Theo":L==="es"?"Próximo Pago":"Your Next Payment Due"}
            </div>
            <div style={{fontSize:"clamp(36px,7vw,54px)",fontWeight:300,color:T.green,letterSpacing:"-0.02em",lineHeight:1}}>
              {fmt(res.nextQ.shortfall)}
            </div>
            <div style={{fontSize:13,color:"#3a8050",marginTop:8}}>
              {L==="vi"?"đến hạn":L==="es"?"con vencimiento":"due by"} <strong style={{color:T.gold}}>{res.nextQ.due}</strong>
            </div>
            {res.catchUp>0&&(
              <div style={{fontSize:11,color:"#5a7050",marginTop:6}}>
                ({L==="vi"?"bao gồm":L==="es"?"incluye":"includes"} {fmt(res.catchUp)} {L==="vi"?"bù đắp quý trước":L==="es"?"de trimestres anteriores":"catch-up"})
              </div>
            )}
          </div>
        ):(
          <div style={{background:T.greenBg,border:"1px solid #1a4020",borderRadius:12,padding:"22px 20px",textAlign:"center",marginBottom:14}}>
            <div style={{fontSize:24,marginBottom:6}}>✓</div>
            <div style={{fontSize:15,fontWeight:500,color:T.green}}>{L==="vi"?"Bạn đã nộp đủ!":L==="es"?"¡Ya pagó suficiente!":"You're fully paid up!"}</div>
          </div>
        )}

        {/* Tax breakdown */}
        <Card>
          <CardTitle>{L==="vi"?"Phân Tích Thuế":L==="es"?"Análisis de Impuestos":"Tax Breakdown"}</CardTitle>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginTop:8}}>
            <tbody>
              {res.incomeType==="self"&&<tr style={{borderBottom:`1px solid ${T.border}`}}>
                <td style={{padding:"8px 0",color:T.textMid}}>SE Tax (15.3%)</td>
                <td style={{padding:"8px 0",textAlign:"right",color:T.text}}>{fmt(res.seTax)}</td>
              </tr>}
              {res.incomeType==="both"&&res.seTax>0&&<tr style={{borderBottom:`1px solid ${T.border}`}}>
                <td style={{padding:"8px 0",color:T.textMid}}>{L==="vi"?"SE Tax thu nhập phụ":L==="es"?"SE Tax ingresos adicionales":"SE Tax on side income"}</td>
                <td style={{padding:"8px 0",textAlign:"right",color:T.text}}>{fmt(res.seTax)}</td>
              </tr>}
              {res.incomeType==="mixed"&&res.seTax>0&&<tr style={{borderBottom:`1px solid ${T.border}`}}>
                <td style={{padding:"8px 0",color:T.textMid}}>{L==="vi"?"SE Tax tự kinh doanh":L==="es"?"SE Tax autónomo":"SE Tax on self-employment"}</td>
                <td style={{padding:"8px 0",textAlign:"right",color:T.text}}>{fmt(res.seTax)}</td>
              </tr>}
              <tr style={{borderBottom:`1px solid ${T.border}`}}>
                <td style={{padding:"8px 0",color:T.textMid}}>{L==="vi"?"Thuế thu nhập ước tính":L==="es"?"Impuesto sobre la renta":"Income tax"}</td>
                <td style={{padding:"8px 0",textAlign:"right",color:T.text}}>{fmt(res.incomeTax)}</td>
              </tr>
              <tr style={{borderBottom:`1px solid ${T.border}`}}>
                <td style={{padding:"8px 0",color:T.gold,fontWeight:600}}>{L==="vi"?"Tổng thuế ước tính":L==="es"?"Total impuesto estimado":"Total estimated tax"}</td>
                <td style={{padding:"8px 0",textAlign:"right",color:T.gold,fontWeight:600}}>{fmt(res.totalTax)}</td>
              </tr>
              <tr style={{borderBottom:`1px solid ${T.border}`}}>
                <td style={{padding:"8px 0",color:T.textMid}}>90% {L==="vi"?"thuế năm nay":L==="es"?"año actual":"current year"}</td>
                <td style={{padding:"8px 0",textAlign:"right",color:T.text}}>{fmt(res.currentSafeHarbor)}</td>
              </tr>
              <tr style={{borderBottom:`1px solid ${T.border}`}}>
                <td style={{padding:"8px 0",color:T.textMid}}>{res.shPct===1.10?"110%":"100%"} {L==="vi"?"thuế năm trước":L==="es"?"año anterior":"prior year"}</td>
                <td style={{padding:"8px 0",textAlign:"right",color:T.text}}>{fmt(res.priorSafeHarbor)}</td>
              </tr>
              <tr>
                <td style={{padding:"10px 0 0",color:T.gold,fontWeight:600}}>
                  {L==="vi"?"Safe Harbor (số nhỏ hơn)":L==="es"?"Puerto Seguro (el menor)":"Safe Harbor (lower of two)"}
                </td>
                <td style={{padding:"10px 0 0",textAlign:"right",color:T.gold,fontWeight:600}}>{fmt(res.safeHarborAmount)}</td>
              </tr>
            </tbody>
          </table>
        </Card>
        ...
        {/* Quarter table */}
        <Card>
          <CardTitle>{L==="vi"?"Chi Tiết Từng Quý":L==="es"?"Detalle por Trimestre":"Quarter-by-Quarter Breakdown"}</CardTitle>
          <div style={{overflowX:"auto",marginTop:10}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:460}}>
              <thead>
                <tr style={{borderBottom:`1px solid ${T.border}`}}>
                  {[L==="vi"?"Quý":L==="es"?"Trim":"Qtr",
                    L==="vi"?"Đáo Hạn":L==="es"?"Vence":"Due",
                    L==="vi"?"Cần Nộp":L==="es"?"Requerido":"Required",
                    L==="vi"?"KT W-2":L==="es"?"Ret. W-2":"W-2 Credit",
                    L==="vi"?"Đã Nộp":L==="es"?"Pagado":"Paid",
                    L==="vi"?"Còn Thiếu":L==="es"?"Faltante":"Still Owe",
                    ""].map((h,i)=>(
                    <th key={i} style={{textAlign:"left",padding:"6px 5px 6px 0",color:T.textDim,fontWeight:400,fontSize:9,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {res.quarters.map((q,i)=>(
                  <tr key={i} style={{borderBottom:`1px solid ${T.border}`}}>
                    <td style={{padding:"9px 5px 9px 0",color:T.gold,fontWeight:500}}>{q.l}</td>
                    <td style={{padding:"9px 5px 9px 0",color:T.textMid,whiteSpace:"nowrap"}}>{q.due}</td>
                    <td style={{padding:"9px 5px 9px 0",color:T.text}}>{fmt(q.required)}</td>
                    <td style={{padding:"9px 5px 9px 0",color:T.textMid}}>{fmt(q.whCredit)}</td>
                    <td style={{padding:"9px 5px 9px 0",color:T.textMid}}>{fmt(q.paid)}</td>
                    <td style={{padding:"9px 5px 9px 0",color:q.shortfall>0?T.red:T.green,fontWeight:q.shortfall>0?600:400}}>
                      {q.shortfall>0?fmt(q.shortfall):"$0 ✓"}
                    </td>
                    <td style={{padding:"9px 5px 9px 0",color:!q.past?T.textDim:q.shortfall>0?T.red:T.green,fontSize:13}}>
                      {!q.past?"→":q.shortfall>0?"⚠":"✓"}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {res.totalPenalty>0&&(
            <IBox tone="red">
              ⚠ {L==="vi"?"Phạt ước tính":L==="es"?"Multa estimada":"Estimated penalty"}: <strong>{fmt(res.totalPenalty)}</strong>
            </IBox>
          )}
        </Card>
        ...
        {/* Payment plan */}
        <Card>
          <CardTitle>{L==="vi"?"Kế Hoạch Thanh Toán":L==="es"?"Plan de Pagos":"Your Payment Plan"}</CardTitle>
          <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>
            {res.quarters.filter(q=>!q.past&&q.shortfall>0).length===0?(
              <div style={{fontSize:13,color:T.green,padding:"8px 0"}}>
                ✓ {L==="vi"?"Tất cả các quý còn lại đã đủ!":L==="es"?"¡Todos los trimestres restantes cubiertos!":"All remaining quarters are covered!"}
              </div>
            ):(
              res.quarters.map((q,i)=>{
                ...
                return(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                    ...
                    border:`1px solid ${q.past&&q.shortfall>0?"#3a1a0a":q.shortfall>0?"#1a4020":T.border}`,
                    opacity:q.shortfall===0&&!q.past?0.6:1}}>
                    <div>
                      <div style={{fontSize:13,color:T.text,fontWeight:500}}>{q.l} · {q.due}</div>
                      <div style={{fontSize:11,color:T.textMid,marginTop:2}}>
                        {q.past&&q.shortfall>0?(L==="vi"?"Đã quá hạn":L==="es"?"Ya venció":"Past due — penalty accruing"):
                         q.shortfall===0?(L==="vi"?"Đã đủ":L==="es"?"Cubierto":"Covered"):
                         (L==="vi"?"Sắp đến hạn":L==="es"?"Próximo":"Upcoming")}
                      </div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontSize:18,color:q.past&&q.shortfall>0?T.red:q.shortfall>0?T.green:T.textDim,fontWeight:600}}>
                        {q.shortfall>0?fmt(q.shortfall):"✓"}
                      </div>
                      {q.shortfall>0&&<div style={{fontSize:10,color:T.textDim}}>{L==="vi"?"cần nộp":L==="es"?"a pagar":"to pay"}</div>}
                    </div>
                  </div>
                );
              })
          ...
          <div style={{marginTop:14,padding:"10px 14px",background:T.bgCard2,borderRadius:7,fontSize:12,color:T.textMid,lineHeight:1.6}}>
            💡 {L==="vi"?"Nộp tại":L==="es"?"Pague en":"Pay at"} <strong style={{color:T.gold}}>irs.gov/payments</strong> → {L==="vi"?"chọn 'Estimated Tax'":L==="es"?"seleccione 'Estimated Tax'":"select 'Estimated Tax'"} → {L==="vi"?"nhập số tiền trên":L==="es"?"ingrese el monto":"enter the amount above"}
          </div>
        </Card>

        <p style={{fontSize:10,color:T.textDim,textAlign:"center",lineHeight:1.6,marginTop:10}}>{TX.disc}</p>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:14}}>
          <Btn ghost small onClick={()=>{setStep(3);setRes(null);}}>
            {L==="vi"?"← Điều chỉnh số liệu":L==="es"?"← Ajustar números":"← Adjust numbers"}
          </Btn>
          <Btn ghost small onClick={()=>{setStep(0);setRes(null);setInp({incomeType:"self",fs:"single",gross:"",expenses:"",w2income:"",w2wh:"",sideIncome:"",sideExpenses:"",rentalIncome:"",rentalExpenses:"",w2mixed:"",w2whMixed:"",selfMixed:"",selfExpMixed:"",rentalMixed:"",priorTax:"",priorAGI:"",unevenIncome:null,q1inc:"",q2inc:"",q3inc:"",q4inc:"",q1paid:"",q2paid:"",q3paid:"",q4paid:""});}}>
            {TX.again}
          </Btn>
        </div>
      </div>)}
  ...
  // Derived
  const netIncome=Math.max(0,parse(inp.gross)-parse(inp.expenses));
  const estimatedTax=estTax(netIncome,inp.fs);
  const priorAGI=parse(inp.priorAGI), thr=getThreshold(inp.fs);
  const shPct=priorAGI>thr?1.10:1.00;
  const priorSafeHarbor=parse(inp.priorTax)*shPct;
  // Use lower of 90% current vs prior safe harbor (when current available)
  const currentSafeHarbor=estimatedTax*0.90;
  const safeHarborAmount=priorSafeHarbor>0
    ? Math.min(currentSafeHarbor>0?currentSafeHarbor:Infinity, priorSafeHarbor)
    : currentSafeHarbor;

  // Today's date to determine which quarters are past
  const today=new Date();
  const currentYear=today.getFullYear();
  // Quarter deadlines (month is 0-indexed)
  const QDATES=[
    new Date(currentYear,3,15),  // Q1 April 15
    new Date(currentYear,5,16),  // Q2 June 16
    new Date(currentYear,8,15),  // Q3 Sept 15
    new Date(currentYear+1,0,15),// Q4 Jan 15 next year
  ];
  const QQ=[
    {l:"Q1",due:L==="vi"?"15/4":L==="es"?"15 abr":"April 15",days:91,mo:L==="vi"?"T1–3":L==="es"?"Ene–Mar":"Jan–Mar",past:today>QDATES[0]},
    {l:"Q2",due:L==="vi"?"16/6":L==="es"?"16 jun":"June 16",days:61,mo:L==="vi"?"T4–5":L==="es"?"Abr–May":"Apr–May",past:today>QDATES[1]},
  ...
  const TX={
    title:L==="vi"?"Máy Tính Thuế Ước Tính Hàng Quý":L==="es"?"Calculadora de Impuesto Trimestral Estimado":"Quarterly Tax Payment Calculator",
    sub:L==="vi"?"Tính chính xác số tiền bạn cần nộp mỗi quý để tránh bị IRS phạt":L==="es"?"Calcule exactamente cuánto pagar cada trimestre para evitar multas del IRS":"Calculate exactly how much to pay each quarter to avoid IRS penalties",
    step1title:L==="vi"?"Loại Thu Nhập":L==="es"?"Tipo de Ingresos":"Your Income Type",
    step2title:L==="vi"?"Thu Nhập Năm Nay":L==="es"?"Ingresos de Este Año":"This Year's Income",
    step3title:L==="vi"?"Thuế Năm Trước":L==="es"?"Impuesto del Año Anterior":"Prior Year Tax",
    step4title:L==="vi"?"Bạn Đã Nộp Bao Nhiêu?":L==="es"?"¿Cuánto Ha Pagado Ya?":"What Have You Paid So Far?",
    incomeTypes:[
      {v:"self",l:L==="vi"?"Tự kinh doanh / 1099":L==="es"?"Autónomo / 1099":"Self-employed / 1099"},
      {v:"both",l:L==="vi"?"W-2 + thu nhập phụ":L==="es"?"W-2 + ingresos adicionales":"W-2 job + side income"},
      {v:"landlord",l:L==="vi"?"Chủ nhà cho thuê":L==="es"?"Propietario de alquiler":"Rental property owner"},
      {v:"mixed",l:L==="vi"?"Kết hợp nhiều loại":L==="es"?"Combinación de varios":"Multiple income types"},
    ],
    fsL:L==="vi"?"Tình trạng khai thuế":L==="es"?"Estado civil":"Filing status",
    FS:[
      {value:"single",label:L==="vi"?"Độc thân":L==="es"?"Soltero/a":"Single"},
    ...
    grossL:L==="vi"?"Tổng thu nhập kinh doanh":L==="es"?"Ingresos brutos del negocio":"Gross business income",
    expL:L==="vi"?"Chi phí kinh doanh":L==="es"?"Gastos del negocio":"Business expenses",
    expH:L==="vi"?"Marketing, văn phòng, xe hơi, v.v.":L==="es"?"Marketing, oficina, vehículo, etc.":"Marketing, office, vehicle, etc.",
    whL:L==="vi"?"Thuế đã khấu trừ từ W-2":L==="es"?"Impuesto retenido del W-2":"W-2 tax withholding",
    whH:L==="vi"?"Nhập 0 nếu không có W-2":L==="es"?"Ingrese 0 si no tiene W-2":"Enter 0 if no W-2 job",
    priorTaxL:L==="vi"?"Thuế liên bang năm trước":L==="es"?"Impuesto federal año anterior":"Prior year federal income tax",
    priorTaxH:L==="vi"?"Mẫu 1040 năm ngoái, Dòng 24":L==="es"?"Formulario 1040 del año pasado, Línea 24":"Last year's Form 1040, Line 24",
    priorAGIL:L==="vi"?"AGI năm trước":L==="es"?"AGI año anterior":"Prior year AGI",
    priorAGIH:L==="vi"?"Mẫu 1040 năm ngoái, Dòng 11 — xác định quy tắc 110%":L==="es"?"Formulario 1040, Línea 11 — determina regla del 110%":"Form 1040, Line 11 — determines 100% vs 110% rule",
    unevenQ:L==="vi"?"Thu nhập của bạn có đều đặn mỗi quý không?":L==="es"?"¿Sus ingresos son similares cada trimestre?":"Is your income roughly the same each quarter?",
    unevenYes:L==="vi"?"Có, khá đều đặn":L==="es"?"Sí, bastante similar":"Yes, fairly even",
    unevenNo:L==="vi"?"Không, thay đổi đáng kể":L==="es"?"No, varía significativamente":"No, it varies significantly",
    qIncL:(q)=>L==="vi"?`Thu nhập ${q}`:L==="es"?`Ingresos ${q}`:`${q} income`,
    qPaidL:(q)=>L==="vi"?`Đã nộp cho ${q}`:L==="es"?`Pagado en ${q}`:`Amount paid for ${q}`,
    qPaidH:L==="vi"?"Nhập 0 nếu chưa nộp":L==="es"?"Ingrese 0 si no pagó":"Enter 0 if not yet paid",
    next:L==="vi"?"Tiếp →":L==="es"?"Siguiente →":"Next →",
    back:L==="vi"?"← Quay lại":L==="es"?"← Atrás":"← Back",
    calc:L==="vi"?"Tính Ngay":L==="es"?"Calcular":"Calculate",
    again:L==="vi"?"Bắt Đầu Lại":L==="es"?"Comenzar de Nuevo":"Start Over",
  ...
  const steps=[TX.step1title,TX.step2title,TX.step3title,TX.step4title];
  const canGo1=!!inp.incomeType;
  const canGo2=parse(inp.gross)>0;
  const canGo3=parse(inp.priorTax)>0&&parse(inp.priorAGI)>0;
  const canGo4=inp.unevenIncome!==null;

  function doCalc(){
    const wh=parse(inp.wh);
    const qPaid=[1,2,3,4].map(i=>parse(inp[`q${i}paid`]));
    const totalPaid=wh+qPaid.reduce((a,b)=>a+b,0);

    // Determine required per quarter
    let qRequired=[];
    if(inp.unevenIncome===false){
      // Even income — equal split of safe harbor
      const perQ=safeHarborAmount/4;
      qRequired=[perQ,perQ,perQ,perQ];
    } else {
      // Uneven income — use Schedule AI per quarter
      // Each period: annualize cumulative income → estimate tax → apply AI_PCT
      const qInc=[1,2,3,4].map(i=>parse(inp[`q${i}inc`]));
      // Cumulative income by period
      const cumInc=[qInc[0], qInc[0]+qInc[1], qInc[0]+qInc[1]+qInc[2], qInc[0]+qInc[1]+qInc[2]+qInc[3]];
      const cumRequired=cumInc.map((ci,i)=>{
        const annualized=ci*AI_FACTORS[i];
        const annTax=estTax(Math.max(0,annualized-parse(inp.expenses)*AI_FACTORS[i]),inp.fs);
        const aiReq=annTax*AI_PCT[i];
        const priorReq=safeHarborAmount*(i+1)/4;
        return Math.min(aiReq,priorReq); // use lower = taxpayer favorable
      });
      // Per quarter = cumulative[i] - cumulative[i-1]
      qRequired=cumRequired.map((cr,i)=>Math.max(0,cr-(i>0?cumRequired[i-1]:0)));
    }

    // Cumulative withholding (spread evenly)
    const perQWH=wh/4;
    ...
    const quarters=QQ.map((q,i)=>{
      ...
      const totalForQ=paid+whCredit;
      const shortfall=Math.max(0,required-totalForQ);
      const overpaid=Math.max(0,totalForQ-required);
      const penalty=q.past?Math.round(shortfall*(IRS_RATE/365)*q.days):0;
      totalPenalty+=penalty;
      return{...q,required,paid,whCredit,totalForQ,shortfall,overpaid,penalty};
    });
    ...
    setRes({
      estimatedTax,netIncome,safeHarborAmount,shPct,
      priorSafeHarbor,currentSafeHarbor,
      totalPaid,totalPenalty,quarters,nextQ,catchUp,
      unevenIncome:inp.unevenIncome,wh,
    });
    setStep(4);
  }
  ...
  return(
    <div style={{maxWidth:600,margin:"0 auto"}}>
      ...
      {step===0&&(<Card>
        <CardTitle>{TX.step1title}</CardTitle>
        <CardSub>{L==="vi"?"Chọn loại thu nhập phù hợp với bạn nhất":L==="es"?"Seleccione el tipo de ingresos que mejor le describe":"Select the income type that best describes you"}</CardSub>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {TX.incomeTypes.map(o=>(
            <button key={o.v} onClick={()=>s("incomeType",o.v)}
              style={{padding:"14px 10px",borderRadius:8,cursor:"pointer",fontFamily:"inherit",fontSize:12,lineHeight:1.4,textAlign:"center",transition:"all 0.15s",
                background:inp.incomeType===o.v?"rgba(200,169,110,0.12)":T.bgCard,
                border:`1px solid ${inp.incomeType===o.v?T.gold:T.border}`,
                color:inp.incomeType===o.v?T.gold:T.textMid}}>
              {o.l}
            </button>
          ))}
        </div>
      ...
      {/* STEP 1 — This year's income */}
      {step===1&&(<Card>
        <CardTitle>{TX.step2title}</CardTitle>
        <CardSub>{L==="vi"?"Chúng tôi sẽ ước tính thuế của bạn tự động":L==="es"?"Estimaremos su impuesto automáticamente":"We'll estimate your tax automatically"}</CardSub>
        <Fld label={TX.grossL}><Inp value={inp.gross} onChange={v=>s("gross",v)} placeholder="0" prefix="$"/></Fld>
        <Fld label={TX.expL} hint={TX.expH} optional optLbl={TX.opt}><Inp value={inp.expenses} onChange={v=>s("expenses",v)} placeholder="0" prefix="$"/></Fld>
        {netIncome>0&&(
          <IBox tone="green">
            {L==="vi"?"Thu nhập ròng":L==="es"?"Ingreso neto":"Net income"}: <strong>{fmt(netIncome)}</strong>
            {" · "}{L==="vi"?"Thuế ước tính":L==="es"?"Impuesto estimado":"Est. tax"}: <strong>{fmt(estimatedTax)}</strong>
            {" · "}90% {L==="vi"?"safe harbor":L==="es"?"puerto seguro":"safe harbor"}: <strong>{fmt(estimatedTax*0.90)}</strong>
          </IBox>
        )}
        <Divider/>
        <Fld label={TX.whL} hint={TX.whH} optional optLbl={TX.opt}><Inp value={inp.wh} onChange={v=>s("wh",v)} placeholder="0" prefix="$"/></Fld>
        <NavRow><Btn ghost onClick={()=>setStep(0)}>{TX.back}</Btn><Btn onClick={()=>setStep(2)} disabled={!canGo}>{TX.next}</Btn></NavRow>
      </Card>)}

      {/* STEP 2 — Prior year safe harbor */}
      {step===2&&(<Card>
        <CardTitle>{TX.step3title}</CardTitle>
        <CardSub>{L==="vi"?"Dùng để tính ngưỡng an toàn (safe harbor)":L==="es"?"Para calcular el umbral de puerto seguro":"Used to calculate your safe harbor amount"}</CardSub>
        <Fld label={TX.priorTaxL} hint={TX.priorTaxH}><Inp value={inp.priorTax} onChange={v=>s("priorTax",v)} placeholder="0" prefix="$"/></Fld>
        ...
        {parse(inp.priorAGI)>0&&parse(inp.priorTax)>0&&(
          <IBox tone={priorAGI>thr?undefined:"green"}>
            {priorAGI>thr
              ?<>AGI &gt; <strong>${thr.toLocaleString()}</strong> → {L==="vi"?"phải nộp":L==="es"?"debe pagar":"must pay"} <strong>110%</strong> = <strong>{fmt(parse(inp.priorTax)*1.10)}</strong></>
              :<>AGI &lt; <strong>${thr.toLocaleString()}</strong> → {L==="vi"?"cần nộp":L==="es"?"requiere":"safe harbor"} <strong>100%</strong> = <strong>{fmt(parse(inp.priorTax))}</strong></>
            }
            {estimatedTax>0&&<> · 90% {L==="vi"?"năm nay":L==="es"?"año actual":"current yr"} = <strong>{fmt(estimatedTax*0.90)}</strong> · {L==="vi"?"Dùng số nhỏ hơn":L==="es"?"Se usa el menor":"Use lower"}: <strong style={{color:T.gold}}>{fmt(safeHarborAmount)}</strong></>}
          </IBox>
        )}
        <NavRow><Btn ghost onClick={()=>setStep(1)}>{TX.back}</Btn><Btn onClick={()=>setStep(3)} disabled={!canGo}>{TX.next}</Btn></NavRow>
      </Card>)}

      {/* STEP 3 — Payments + income pattern */}
      {step===3&&(<Card>
        <CardTitle>{TX.step4title}</CardTitle>
        <CardSub>{L==="vi"?"Nhập những gì bạn đã nộp cho IRS (không bao gồm khấu trừ W-2)":L==="es"?"Ingrese lo que ya pagó al IRS (sin incluir retenciones W-2)":"Enter what you've paid the IRS directly (not W-2 withholding)"}</CardSub>
        ...
        {inp.unevenIncome===true&&(
          <div style={{marginBottom:14,padding:"12px 14px",background:T.bgCard2,border:`1px solid ${T.border}`,borderRadius:8}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {QQ.map((q,i)=>(
                <Fld key={i} label={`${q.l} · ${q.mo}`}>
                  <Inp value={inp[`q${i+1}inc`]} onChange={v=>s(`q${i+1}inc`,v)} placeholder="0" prefix="$"/>
                </Fld>
              ))}
            </div>
          </div>
        ...
        {inp.unevenIncome!==null&&(<>
          <Divider/>
          <p style={{fontSize:11,color:T.textMid,margin:"0 0 10px",fontWeight:500}}>
            {L==="vi"?"Số tiền đã nộp cho mỗi quý (chỉ thanh toán trực tiếp IRS):":L==="es"?"Monto pagado por trimestre (solo pagos directos al IRS):":"Amount paid each quarter (direct IRS payments only):"}
          </p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {QQ.map((q,i)=>(
              <Fld key={i} label={`${q.l} · ${q.due}`}
        ...
        <NavRow>
          <Btn ghost onClick={()=>setStep(2)}>{TX.back}</Btn>
          <Btn onClick={doCalc} disabled={!canGo}>{TX.calc}</Btn>
        </NavRow>
      </Card>)}
      ...
      {step===4&&res&&(<div>
        ...
        {res.nextQ&&res.nextQ.shortfall>0?(
          <div style={{background:"linear-gradient(135deg,#0d1a08,#0a1505)",border:"1px solid #1a4020",borderRadius:12,padding:"24px 20px",textAlign:"center",marginBottom:14}}>
            ...
            {res.catchUp>0&&(
              <div style={{fontSize:11,color:"#5a7050",marginTop:6}}>
                ({L==="vi"?"bao gồm":L==="es"?"incluye":"includes"} {fmt(res.catchUp)} {L==="vi"?"bù đắp cho quý trước":L==="es"?"de recuperación de trimestres anteriores":"catch-up for past quarters"})
              </div>
            )}
          </div>
        ):(
          <div style={{background:T.greenBg,border:"1px solid #1a4020",borderRadius:12,padding:"22px 20px",textAlign:"center",marginBottom:14}}>
            <div style={{fontSize:24,marginBottom:6}}>✓</div>
            <div style={{fontSize:15,fontWeight:500,color:T.green}}>
              {L==="vi"?"Bạn đã nộp đủ!":L==="es"?"¡Ya pagó suficiente!":"You're fully paid up!"}
            </div>
            <div style={{fontSize:12,color:"#3a6040",marginTop:4}}>
              {L==="vi"?"Không cần nộp thêm cho đến quý tiếp theo":L==="es"?"No necesita pagar más hasta el próximo trimestre":"No additional payments needed until the next quarter"}
            </div>
          </div>
        )}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
          {[
            ...
            {l:L==="vi"?"Tổng Đã Nộp":L==="es"?"Total Pagado":"Total Paid",v:fmt(res.totalPaid)},
            {l:L==="vi"?"Tiền Phạt":L==="es"?"Multa Est.":"Est. Penalty",v:fmt(res.totalPenalty),hi:res.totalPenalty>0},
          ].map((it,i)=>(
            <div key={i} style={{background:T.bgCard,border:`1px solid ${it.hi?T.goldDim:T.border}`,borderRadius:8,padding:"11px 12px"}}>
              <div style={{fontSize:9,color:T.textDim,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:3}}>{it.l}</div>
              <div style={{fontSize:16,color:it.hi?T.red:T.text,fontWeight:300}}>{it.v}</div>
            </div>
          ))}
        </div>

        <IBox>
          {L==="vi"?"Safe harbor":L==="es"?"Puerto seguro":"Safe harbor"}: {res.shPct===1.10?"110%":"100%"} {L==="vi"?"thuế năm trước":L==="es"?"impuesto año anterior":"prior year tax"} ({fmt(res.priorSafeHarbor)}){estimatedTax>0?` vs 90% ${L==="vi"?"thuế năm nay":L==="es"?"año actual":"current yr"} (${fmt(res.currentSafeHarbor)})`:""}. {L==="vi"?"Dùng số nhỏ hơn":L==="es"?"Se usa el menor":"Lower used"}: <strong style={{color:T.gold}}>{fmt(res.safeHarborAmount)}</strong>
        </IBox>
        ...
        <Card style={{marginTop:12}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:460}}>
              <thead>
                <tr style={{borderBottom:`1px solid ${T.border}`}}>
                  {[
                    L==="vi"?"Quý":L==="es"?"Trim":"Qtr",
                    L==="vi"?"Đáo Hạn":L==="es"?"Vence":"Due Date",
                    L==="vi"?"Cần Nộp":L==="es"?"Requerido":"Required",
                    L==="vi"?"W-2 KT":L==="es"?"Retención":"W-2 Credit",
                    L==="vi"?"Đã Nộp":L==="es"?"Pagado":"Paid",
                    L==="vi"?"Còn Thiếu":L==="es"?"Faltante":"Still Owe",
                    L==="vi"?"Tình Trạng":L==="es"?"Estado":"Status",
                  ].map(h=>(
                    <th key={h} style={{textAlign:"left",padding:"6px 5px 6px 0",color:T.textDim,fontWeight:400,fontSize:9,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {res.quarters.map((q,i)=>{
                  const isPast=q.past;
                  const statusIcon=!isPast?"→":q.shortfall>0?"⚠":"✓";
                  const statusColor=!isPast?T.textDim:q.shortfall>0?T.red:T.green;
                  return(
                    <tr key={i} style={{borderBottom:`1px solid ${T.border}`}}>
                      <td style={{padding:"9px 5px 9px 0",color:T.gold,fontWeight:500}}>{q.l}</td>
                      <td style={{padding:"9px 5px 9px 0",color:T.textMid,whiteSpace:"nowrap"}}>{q.due}</td>
                      ...
                      <td style={{padding:"9px 5px 9px 0",color:statusColor,fontSize:13}}>{statusIcon}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          {res.totalPenalty>0&&(
            <IBox tone="red">
              ⚠ {L==="vi"?"Phạt ước tính các quý đã qua":L==="es"?"Multa por trimestres vencidos":"Estimated penalty for past underpayments"}: <strong>{fmt(res.totalPenalty)}</strong> — {L==="vi"?"nộp đúng hạn các quý còn lại để tránh thêm phạt":L==="es"?"pague a tiempo los trimestres restantes para evitar más multas":"pay remaining quarters on time to avoid additional penalties"}
            </IBox>
          )}
        </Card>

        {/* Payment plan — action focused */}
        <Card>
          <CardTitle>{L==="vi"?"Kế Hoạch Thanh Toán":L==="es"?"Plan de Pagos":"Your Payment Plan"}</CardTitle>
          <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>
            {res.quarters.filter(q=>!q.past&&q.shortfall>0).length===0?(
              <div style={{fontSize:13,color:T.green,padding:"8px 0"}}>
                ✓ {L==="vi"?"Tất cả các quý còn lại đã được thanh toán đầy đủ!":L==="es"?"¡Todos los trimestres restantes ya están cubiertos!":"All remaining quarters are fully covered!"}
              </div>
            ):(
              res.quarters.map((q,i)=>{
                if(q.past&&q.shortfall===0)return null;
                if(!q.past&&q.shortfall===0)return(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:T.bgCard2,border:`1px solid ${T.border}`,borderRadius:8,opacity:0.6}}>
                    <span style={{fontSize:13,color:T.textMid}}>{q.l} · {q.due}</span>
                    <span style={{fontSize:13,color:T.green}}>✓ {L==="vi"?"Đã đủ":L==="es"?"Cubierto":"Covered"}</span>
                  </div>
                );
                return(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",
                    background:q.past?T.redBg:T.greenBg,
                    border:`1px solid ${q.past?"#3a1a0a":"#1a4020"}`,borderRadius:8}}>
                    <div>
                      <div style={{fontSize:13,color:T.text,fontWeight:500}}>{q.l} · {q.due}</div>
                      <div style={{fontSize:11,color:T.textMid,marginTop:2}}>{q.mo}{q.past?` · ${L==="vi"?"Đã quá hạn":L==="es"?"Ya venció":"Past due"}`:`  · ${L==="vi"?"Sắp đến hạn":L==="es"?"Próximo":"Upcoming"}`}</div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontSize:18,color:q.past?T.red:T.green,fontWeight:600}}>{fmt(q.shortfall)}</div>
                      <div style={{fontSize:10,color:T.textDim}}>{L==="vi"?"cần nộp":L==="es"?"a pagar":"to pay"}</div>
                    </div>
                  </div>
                );
              })
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:14}}>
          ...
          <Btn ghost small onClick={()=>{setStep(0);setRes(null);setInp({incomeType:"self",fs:"single",gross:"",expenses:"",wh:"",priorTax:"",priorAGI:"",unevenIncome:null,q1inc:"",q2inc:"",q3inc:"",q4inc:"",q1paid:"",q2paid:"",q3paid:"",q4paid:""});}}>
            {TX.again}
          </Btn>
        </div>
      </div>)}
