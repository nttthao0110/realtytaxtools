...
function DepreciationCalc({lang:L="en"}){
  ...
  const [inp,setInp]=useState({
    // Quick mode
    quickTotalClosing:"",quickLandPct:"",
    // A
    purchasePrice:"",
    // B — added to basis
    ...
    pocAppraisal:"",pocInspection:"",pocOther:"",pocDestination:"Property Basis (B)",
    // C — loan costs
    ...
    taxProrationType:"",  // "pay" = buyer pays taxes, "credit" = seller credits buyer
  ...
  const quickBasis=parse(inp.purchasePrice)+parse(inp.quickTotalClosing);
  const quickLandRatio=Math.min(1,Math.max(0,parse(inp.quickLandPct)/100));
  const quickBuildingBasis=Math.max(0,quickBasis*(1-quickLandRatio));
  const quickAnnualDepr=quickBuildingBasis>0?Math.round(quickBuildingBasis/27.5*100)/100:0;
  ...
  async function handlePDF(e){
    ...
    try{
      ...
      // Call our serverless proxy — API key stays server-side
      ...
      ["sectionB","sectionC","sectionD","sectionE","sectionF"].forEach(sec=>{
        const obj=d[sec]||{};
        Object.entries(obj).forEach(([k,v])=>{if(v!=null&&v!==0&&v!=="") s(k,String(v));});
      });
      ...
      setPdfFlags(
        (d.flags||[]).length>0
          ? d.flags
          : ["✓ PDF read successfully — review all fields below and correct any errors."]
      );
    }catch(err){
  ...
  return(
    <div>
      ...
      {step===0&&(<div>
        ...
        {/* A — Purchase Price */}
        <SB color="#2a5a3a" tc="#90d4a0" title="A — Purchase Price" defaultOpen={true}>
          <MF label={"Purchase price (from sales contract / HUD-1 Line 101)"} value={inp.purchasePrice} onChange={v=>s("purchasePrice",v)}/>
        </SB>

        {/* Mode toggle */}
        <div style={{display:"flex",gap:6,marginBottom:12,padding:"4px",background:T.bgCard,borderRadius:8,border:"1px solid "+T.border}}>
          {[{v:true,l:"⚡ Quick Mode",d:"4 fields, instant result"},{v:false,l:"📋 Detailed Mode",d:"Full HUD-1 breakdown"}].map(m=>(
            <button key={String(m.v)} onClick={()=>setQuickMode(m.v)}
              style={{flex:1,padding:"8px",borderRadius:6,cursor:"pointer",fontFamily:"inherit",border:"none",
                background:quickMode===m.v?"linear-gradient(135deg,"+T.gold+","+T.goldLight+")":T.bgCard2,
                color:quickMode===m.v?"#050608":T.textMid,transition:"all 0.15s"}}>
              <div style={{fontSize:12,fontWeight:quickMode===m.v?700:400}}>{m.l}</div>
              <div style={{fontSize:9,opacity:0.7,marginTop:1}}>{m.d}</div>
            </button>
          ))}
        </div>
        ...
        {quickMode&&(<div>
          <SB color="#2a5a3a" tc="#90d4a0" title="A — Purchase Price" defaultOpen={true}>
            <MF label={"Purchase price"} value={inp.purchasePrice} onChange={v=>s("purchasePrice",v)}/>
          </SB>
          <SB color="#2a3a6a" tc="#90b0f0" title="B — Total Closing Costs Added to Basis" defaultOpen={true}>
            <MF label={"Total closing costs (title, recording, settlement, attorney, survey, etc.)"} hint="Sum everything from your HUD-1 that is NOT loan costs, prepaids, or escrow. When in doubt, switch to Detailed Mode." value={inp.quickTotalClosing} onChange={v=>s("quickTotalClosing",v)}/>
          </SB>
          <div style={{padding:"10px 14px",background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,marginBottom:8}}>
            <div style={{fontSize:10,color:T.gold,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>{"Land Ratio"}</div>
            <Fld label={"Land % of purchase price"} hint={"From county assessor — land cannot be depreciated. Typical range 15–35%."}>
              <Inp value={inp.quickLandPct} onChange={v=>s("quickLandPct",v)} placeholder="25"/>
            </Fld>
          </div>
          {quickBasis>0&&quickLandRatio>0&&(
            <div style={{background:"linear-gradient(135deg,#0a1a0e,#080e0a)",border:"1px solid #1a4020",borderRadius:10,padding:"14px 16px",marginBottom:12}}>
              <p style={{fontSize:10,color:T.green,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",margin:"0 0 10px"}}>{"Quick Depreciation Estimate"}</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div>
                  <div style={{fontSize:9,color:T.textDim,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>{"Total Basis"}</div>
                  <div style={{fontSize:22,color:T.gold,fontWeight:300}}>{fmt(quickBasis)}</div>
                </div>
                <div>
                  <div style={{fontSize:9,color:T.textDim,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>{"Building Basis"}</div>
                  <div style={{fontSize:22,color:T.gold,fontWeight:300}}>{fmt(quickBuildingBasis)}</div>
                  <div style={{fontSize:9,color:T.textDim}}>{pct(1-quickLandRatio)+" of basis"}</div>
                </div>
                <div>
                  <div style={{fontSize:9,color:T.textDim,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>{"Annual Depreciation"}</div>
                  <div style={{fontSize:26,color:T.green,fontWeight:300}}>{fmt(quickAnnualDepr)}</div>
                  <div style={{fontSize:9,color:T.textDim}}>{"27.5yr straight-line"}</div>
                </div>
                <div>
                  <div style={{fontSize:9,color:T.textDim,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2}}>{"Tax Savings @ 22%"}</div>
                  <div style={{fontSize:26,color:T.green,fontWeight:300}}>{fmt(Math.round(quickAnnualDepr*0.22))}</div>
                  <div style={{fontSize:9,color:T.textDim}}>{"per year (estimate)"}</div>
                </div>
              </div>
              <div style={{marginTop:10,padding:"6px 10px",background:"rgba(74,144,96,0.08)",borderRadius:4,fontSize:10,color:T.textDim}}>
                {"For exact depreciation with asset classes, cost seg, and bonus depreciation — switch to Detailed Mode and proceed through all 4 steps."}
              </div>
            </div>
          )}
          <p style={{fontSize:9,color:T.textDim,textAlign:"center",marginTop:4}}>{"Estimate only — uses 27.5yr straight-line. Switch to Detailed Mode for full analysis."}</p>
        </div>)}
        ...
        <SB color="#2a3a6a" tc="#90b0f0" title="B — HUD-1 Expenses Added to Property Cost Basis (capitalized — NOT immediately deductible)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            <MF label={"Title Insurance — Lender's Policy"} hint="HUD-1 line 1101" value={inp.titleInsuranceLender} onChange={v=>s("titleInsuranceLender",v)}/>
            <MF label={"Title Insurance — Owner's Policy"} hint="HUD-1 line 1103" value={inp.titleInsuranceOwner} onChange={v=>s("titleInsuranceOwner",v)}/>
            <MF label={"Title Search / Abstract / Title Exam"} hint="HUD-1 line 1102" value={inp.titleSearch} onChange={v=>s("titleSearch",v)}/>
            <MF label={"Other Title Fees"} hint="Endorsements, binder, etc." value={inp.otherTitleFees} onChange={v=>s("otherTitleFees",v)}/>
            <MF label={"Settlement / Closing Fee"} hint="HUD-1 line 1100" value={inp.settlementFee} onChange={v=>s("settlementFee",v)}/>
            ...
            <MF label={"Attorney Fees — Title/Closing (adds to BASIS)"} hint="Title exam, deed prep, closing attorney. Listed in settlement section of HUD-1. If your attorney fee is in the LOAN section of HUD-1, enter it in Section C instead." value={inp.attorneyFeesBasis} onChange={v=>s("attorneyFeesBasis",v)}/>
            ...
            <MF label={"Appraisal — Buyer's Own (NOT required by lender)"} hint="Only if you got an independent appraisal not required by lender. If lender required it, enter in Section C. Adds to basis." value={inp.appraisalBasis} onChange={v=>s("appraisalBasis",v)}/>
            <MF label={"Other Closing Costs"} hint="Home warranty, buyer agent commission, etc." value={inp.otherBasis} onChange={v=>s("otherBasis",v)}/>
          </div>
        ...
        {hasloan&&<SB color="#5a3a6a" tc="#c090d8" title="C — HUD-1 Expenses — Loan Cost Basis (amortize over loan term)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            <MF label={"Loan Origination Fee"} hint="HUD-1 line 801" value={inp.originationFee} onChange={v=>s("originationFee",v)}/>
            <MF label={"Loan Discount / Points"} hint="HUD-1 line 802" value={inp.discountPoints} onChange={v=>s("discountPoints",v)}/>
            <MF label={"Appraisal — Required by Lender (on HUD-1 statement)"} hint="If lender required the appraisal and it appears on the HUD-1. If paid outside closing (POC), use the POC section below instead." value={inp.appraisalLender} onChange={v=>s("appraisalLender",v)}/>
            <MF label={"Credit Report"} hint="HUD-1 line 804" value={inp.creditReport} onChange={v=>s("creditReport",v)}/>
            ...
            <MF label={"Attorney Fees — Loan Section Only (NOT basis)"} hint="ONLY if listed in the loan costs section of HUD-1. If listed in settlement/title section, enter in Section B above." value={inp.attorneyFeeLoan} onChange={v=>s("attorneyFeeLoan",v)}/>
          ...
          <div style={{borderTop:"1px solid "+T.border,marginTop:10,paddingTop:10}}>
            <MF label={"Lender Credit to Buyer → Enter as POSITIVE"}
              hint="Reduces loan cost basis and reduces cash to close" value={inp.lenderCredit} onChange={v=>s("lenderCredit",v)}/>
          </div>
        ...
        <SB color="#1a4a3a" tc="#70d0a0" title="D — HUD-1 Expenses Currently Deductible (deduct in year of closing)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            <div style={{marginBottom:10}}>
              <div style={{fontSize:11,color:T.textMid,marginBottom:4}}>{"Property Tax at Closing"}</div>
              <div style={{fontSize:9,color:T.textDim,marginBottom:6,lineHeight:1.5}}>
                {"Look at your HUD-1: is the tax proration a CHARGE to you (you pay it) or a CREDIT to you (seller pays you)? Enter in ONE place only."}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                {[
                  {v:"pay",l:"I PAY taxes at closing",d:"Listed as a charge/debit to buyer → deductible (Section D)"},
                  {v:"credit",l:"Seller CREDITS me",d:"Listed as a credit to buyer → reduces my basis (Section F)"},
                ].map(o=>(
                  <button key={o.v} onClick={()=>s("taxProrationType",o.v)}
                    style={{padding:"8px 10px",borderRadius:6,cursor:"pointer",fontFamily:"inherit",textAlign:"left",
                      background:inp.taxProrationType===o.v?"rgba(200,169,110,0.1)":T.bgCard,
                      border:"1px solid "+(inp.taxProrationType===o.v?T.gold:T.border)}}>
                    <div style={{fontSize:11,color:inp.taxProrationType===o.v?T.gold:T.textMid,fontWeight:600}}>{o.l}</div>
                    <div style={{fontSize:9,color:T.textDim,marginTop:2}}>{o.d}</div>
                  </button>
                ))}
              </div>
              {inp.taxProrationType==="pay"&&(
                <Inp value={inp.propertyTaxClosing} onChange={v=>{s("propertyTaxClosing",v);s("taxAdjSeller","");}} placeholder="0" prefix="$"/>
              )}
              {inp.taxProrationType==="credit"&&(
                <div>
                  <div style={{fontSize:10,color:T.amber,marginBottom:4}}>{"Amount of seller's tax credit → reduces your basis"}</div>
                  <Inp value={inp.taxAdjSeller} onChange={v=>{s("taxAdjSeller",v);s("propertyTaxClosing","");}} placeholder="0" prefix="$"/>
                </div>
              )}
              {!inp.taxProrationType&&(
                <div style={{padding:"6px 10px",background:"rgba(200,169,110,0.04)",borderRadius:4,fontSize:10,color:T.textDim}}>
                  {"Select whether you are paying or receiving a tax credit above"}
                </div>
              )}
            </div>
            <MF label={"Prepaid Mortgage Interest ← DEDUCTIBLE"} hint="Closing date to month-end · Schedule E" value={inp.prepaidInterest} onChange={v=>s("prepaidInterest",v)}/>
            <MF label={"Homeowners Insurance + Upfront MIP/PMI ← DEDUCTIBLE"} hint="Full premium deductible (cash basis · IRS Rev. Rul. 2007-3)" value={inp.insuranceMIP} onChange={v=>s("insuranceMIP",v)}/>
            <MF label={"Prorated Rent from Seller → TAXABLE INCOME"} hint="Report on Schedule E · enter positive" income value={inp.proratedRent} onChange={v=>s("proratedRent",v)}/>
          </div>
        ...
        <SB color="#3a2a5a" tc="#a080c8" title="E — Reserves and Deposits (escrow — deductible only when paid out)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            <MF label={"Deposit with Lender — Homeowners Insurance"} value={inp.escrowInsurance} onChange={v=>s("escrowInsurance",v)}/>
            <MF label={"Deposit with Lender — Property Tax"} value={inp.escrowTax} onChange={v=>s("escrowTax",v)}/>
            <MF label={"Deposit with Lender — Mortgage Insurance"} value={inp.escrowMortgageIns} onChange={v=>s("escrowMortgageIns",v)}/>
            <MF label={"Aggregate Adjustment Credit → Enter as POSITIVE"}
              hint="Lender credit to prevent escrow overfunding — reduces cash to close" value={inp.aggregateAdj} onChange={v=>s("aggregateAdj",v)}/>
          </div>
        </SB>
        ...
        <SB color="#5a2a1a" tc="#e09060" title="F — Reductions to Amount Due (all enter as POSITIVE)">
          <div style={{display:"grid",gridTemplateColumns:"1fr",gap:0}}>
            ...
            {/* taxAdjSeller is now handled in Section D tax proration selector above */}
            {inp.taxProrationType==="credit"&&inp.taxAdjSeller&&(
              <div style={{padding:"6px 10px",background:"rgba(200,169,110,0.06)",borderRadius:4,fontSize:11,color:T.amber,marginBottom:8}}>
                {"Tax proration credit: "+fmt(parse(inp.taxAdjSeller))+" → entered in Section D, reducing your basis here in Section F automatically"}
              </div>
            )}
        ...
        {quickMode&&(<NavRow><div/><Btn onClick={()=>{setQuickMode(false);}} style={{}}>{'Switch to Detailed Mode →'}</Btn></NavRow>)}
        {!quickMode&&(<NavRow><div/><Btn onClick={()=>setStep(1)} disabled={!canGo0}>{"Next →"}</Btn></NavRow>)}
      </div>)}
