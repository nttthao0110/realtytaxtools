...
function IBox({children,tone}){
  ...
  return <div style={{background:c.bg,border:`1px solid ${c.border}`,borderRadius:6,padding:"12px 14px",marginTop:12,fontSize:12,color:c.text,lineHeight:1.7}}>{children}</div>;
}
...
function UnderpaymentCalc({lang}){
  ...
  const [useAI,setAI]=useState(false);
  const [inp,setInp]=useState({kt:null,fs:"single",gross:"",tax:"",wh:"",prior:"",agi:"",q1:"",q2:"",q3:"",q4:"",a1:"",a2:"",a3:"",a4:"",aw1:"",aw2:"",aw3:"",aw4:""});
  const [res,setRes]=useState(null);
  const s=(k,v)=>setInp(p=>({...p,[k]:v}));
  const priorAGI=parse(inp.agi), thr=getThreshold(inp.fs), shPct=priorAGI>thr?1.10:1.00;
  const effTax=inp.kt===true?parse(inp.tax):inp.kt===false?estTax(parse(inp.gross),inp.fs):null;
  const canGo=inp.kt==="skip"||(inp.kt===true&&parse(inp.tax)>0)||(inp.kt===false&&parse(inp.gross)>0);
  const QQ=[
    {l:"Q1",due:L==="vi"?"15/4":L==="es"?"15 abr":"April 15",days:91,mo:L==="vi"?"T1–3":L==="es"?"Ene–Mar":"Jan–Mar"},
    {l:"Q2",due:L==="vi"?"16/6":L==="es"?"16 jun":"June 16",days:61,mo:L==="vi"?"T4–5":L==="es"?"Abr–May":"Apr–May"},
    {l:"Q3",due:L==="vi"?"15/9":L==="es"?"15 sep":"Sept 15",days:91,mo:L==="vi"?"T6–8":L==="es"?"Jun–Ago":"Jun–Aug"},
    {l:"Q4",due:L==="vi"?"15/1":L==="es"?"15 ene":"Jan 15",days:121,mo:L==="vi"?"T9–12":L==="es"?"Sep–Dic":"Sep–Dec"},
  ];
  const FS=[
    {value:"single",label:L==="vi"?"Độc thân":L==="es"?"Soltero/a":"Single"},
    {value:"mfj",label:L==="vi"?"VCK chung":L==="es"?"Casado conjunto":"Married Joint"},
    {value:"mfs",label:L==="vi"?"VCK riêng":L==="es"?"Casado separado":"Married Separate"},
    {value:"hoh",label:L==="vi"?"Chủ hộ":L==="es"?"Jefe de hogar":"Head of Household"},
  ];
  const TX={
    s0q:L==="vi"?"Bạn có biết thuế năm nay không?":L==="es"?"¿Conoce su impuesto del año actual?":"Do you know your current year tax?",
    yes:L==="vi"?"Có, tôi biết":L==="es"?"Sí, lo sé":"Yes, I know it",
    est:L==="vi"?"Ước tính cho tôi":L==="es"?"Estimarlo":"Estimate it",
    skip:L==="vi"?"Bỏ qua":L==="es"?"Omitir":"Skip this step",
    taxL:L==="vi"?"Tổng thuế liên bang":L==="es"?"Total impuesto federal":"Federal income tax",
    taxH:L==="vi"?"Mẫu 1040, Dòng 24":L==="es"?"Formulario 1040, Línea 24":"Form 1040, Line 24",
    fsL:L==="vi"?"Tình trạng khai thuế":L==="es"?"Estado civil":"Filing status",
    incL:L==="vi"?"Tổng thu nhập hàng năm":L==="es"?"Ingreso bruto anual":"Gross annual income",
    incH:L==="vi"?"Khấu trừ tiêu chuẩn sẽ được áp dụng":L==="es"?"Se aplicará deducción estándar":"Standard deduction will be applied",
    whL:L==="vi"?"Thuế đã khấu trừ":L==="es"?"Impuesto retenido":"Federal tax withheld",
    whH:L==="vi"?"W-2 Ô 2 + khấu trừ 1099":L==="es"?"W-2 Casilla 2 + retención 1099":"W-2 Box 2 + 1099 withholding",
    skipInfo:L==="vi"?"Ngưỡng an toàn sẽ tính từ thuế năm trước.":L==="es"?"Se usará solo el impuesto del año anterior.":"Safe harbor calculated from prior year tax only.",
    priorL:L==="vi"?"Thuế liên bang năm trước":L==="es"?"Impuesto federal año anterior":"Prior year federal tax",
    priorH:L==="vi"?"Mẫu 1040 năm ngoái, Dòng 24":L==="es"?"Formulario 1040 del año pasado, Línea 24":"Last year's Form 1040, Line 24",
    agiL:L==="vi"?"AGI năm trước":L==="es"?"AGI año anterior":"Prior year AGI",
    agiH:L==="vi"?"Mẫu 1040, Dòng 11":L==="es"?"Formulario 1040, Línea 11":"Form 1040, Line 11",
    s2sub:L==="vi"?"Chỉ thanh toán trực tiếp cho IRS":L==="es"?"Solo pagos directos al IRS":"Direct IRS payments only",
    s2zero:L==="vi"?"Để $0 nếu không có thanh toán trong quý đó.":L==="es"?"Deje $0 si no realizó pago ese trimestre.":"Leave $0 for quarters with no payment.",
    aiToggle:L==="vi"?"Dùng Lịch AI — Thu Nhập Không Đều":L==="es"?"Usar Horario AI — Ingresos Irregulares":"Use Schedule AI — Annualized Income",
    aiDesc:L==="vi"?"Cho thu nhập biến động. Tính từng quý theo thu nhập thực tế.":L==="es"?"Para ingresos variables. Calcula cada trimestre según ingresos reales.":"For uneven income. Calculates each quarter based on actual earnings.",
    s3sub:L==="vi"?"Nhập thu nhập lũy kế đến cuối mỗi kỳ IRS":L==="es"?"Ingrese ingresos acumulados por período IRS":"Enter cumulative income through end of each IRS period",
    aiIncL:L==="vi"?"Thu nhập lũy kế":L==="es"?"Ingresos acumulados":"Cumulative income",
    aiWhL:L==="vi"?"Khấu trừ lũy kế":L==="es"?"Retención acumulada":"Cumulative withholding",
    next:L==="vi"?"Tiếp →":L==="es"?"Siguiente →":"Next →",
    back:L==="vi"?"← Quay lại":L==="es"?"← Atrás":"← Back",
    calc:L==="vi"?"Tính Tiền Phạt":L==="es"?"Calcular Multa":"Calculate Penalty",
    nextAI:L==="vi"?"Tiếp: Nhập Thu Nhập →":L==="es"?"Siguiente: Ingresos →":"Next: Enter Income →",
    again:L==="vi"?"Bắt Đầu Lại":L==="es"?"Comenzar de Nuevo":"Start Over",
    opt:L==="vi"?"tùy chọn":L==="es"?"opcional":"optional",
    rMet:L==="vi"?"✓ Đạt Ngưỡng An Toàn":L==="es"?"✓ Puerto Seguro Cumplido":"✓ Safe Harbor Met — No Penalty",
    rPen:L==="vi"?"⚠ Ước Tính Tiền Phạt":L==="es"?"⚠ Multa Estimada":"⚠ Estimated Underpayment Penalty",
    rAI:L==="vi"?"Lịch AI · Hàng Năm Hóa":L==="es"?"Horario AI · Anualizado":"Schedule AI · Annualized",
    rPaid:L==="vi"?"Tổng Đã Nộp":L==="es"?"Total Pagado":"Total Paid",
    rSH:L==="vi"?"Ngưỡng An Toàn":L==="es"?"Puerto Seguro":"Safe Harbor",
    rShort:L==="vi"?"Thiếu Hụt":L==="es"?"Déficit":"Shortfall",
    rPenLbl:L==="vi"?"Tiền Phạt":L==="es"?"Multa Est.":"Est. Penalty",
    rSafeTitle:L==="vi"?"Tính Ngưỡng An Toàn":L==="es"?"Cálculo Puerto Seguro":"Safe Harbor Breakdown",
    rUsed:L==="vi"?"Ngưỡng sử dụng":L==="es"?"Umbral usado":"Threshold used",
    rLesser:L==="vi"?"(nhỏ hơn)":L==="es"?"(el menor)":"(lesser of two)",
    rNP:L==="vi"?"chưa cung cấp":L==="es"?"no proporcionado":"not provided",
    rQTitle:L==="vi"?"Chi Tiết Hàng Quý":L==="es"?"Desglose Trimestral":"Quarterly Breakdown",
    rAITitle:L==="vi"?"Chi Tiết Lịch AI":L==="es"?"Desglose Horario AI":"Schedule AI Breakdown",
    rTips:L==="vi"?"Cách Tránh Khoản Phạt":L==="es"?"Cómo Evitar Esta Multa":"How to Avoid This Penalty",
  ...
  const steps=useAI
    ?(L==="vi"?["Năm HT","Năm Trước","Thanh Toán","Lịch AI","KQ"]:L==="es"?["Año Act","Año Ant","Pagos","Horario AI","Result"]:["Current","Prior","Payments","Sched. AI","Results"])
    :(L==="vi"?["Năm HT","Năm Trước","Thanh Toán","Kết Quả"]:L==="es"?["Año Act","Año Ant","Pagos","Resultados"]:["Current","Prior","Payments","Results"]);
  const dStep=Math.min(step,steps.length-1);

  function doCalc(){
    const prior=parse(inp.prior),wh=parse(inp.wh),qp=[1,2,3,4].map(i=>parse(inp[`q${i}`]));
    const sp=prior*shPct, sc=effTax!=null?effTax*.9:null;
    const shMin=sc!=null?Math.min(sp,sc):sp;
    const win=sc!=null&&sc<sp?"current":"prior";
    const totalPaid=wh+qp.reduce((a,b)=>a+b,0);
    let tp=0,qd;
    if(useAI){
      const ai=[1,2,3,4].map(i=>parse(inp[`a${i}`]));
      const aw=[1,2,3,4].map(i=>parse(inp[`aw${i}`]));
      qd=QQ.map((q,i)=>{
        const aInc=ai[i]*AI_FACTORS[i],aTax=estTax(aInc,inp.fs);
        const cr=aTax*AI_PCT[i],pr=sp*(i+1)/4,er=Math.min(cr,pr);
        const cw=aw[i]||(wh*(i+1)/4),cp=cw+qp.slice(0,i+1).reduce((a,b)=>a+b,0);
        const qs=Math.max(0,er-cp),pen=qs*(IRS_RATE/365)*q.days;
        tp+=pen;
        return{...q,aInc,aTax,cr,pr,er,shortfall:qs,penalty:pen,required:er};
      });
    } else {
      const qr=shMin/4;
      qd=QQ.map((q,i)=>{
        const cp=wh/4+qp.slice(0,i+1).reduce((a,b)=>a+b,0);
        const qs=Math.max(0,qr*(i+1)-cp),pen=qs*(IRS_RATE/365)*q.days;
        tp+=pen;
        return{...q,shortfall:qs,penalty:pen,required:qr};
      });
    }
    const met=useAI?tp===0:totalPaid>=shMin;
    setRes({prior,wh,qp,totalPaid,sp,sc,shMin,shPct,win,shortfall:Math.max(0,shMin-totalPaid),
      totalPenalty:met?0:tp,met,qd,useAI,effTax,est:inp.kt===false});
  ...
  return(
    <div style={{maxWidth:580,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",gap:5,marginBottom:24}}>
        {steps.map((s,i)=>{const a=i===dStep,d=i<dStep;return(
          <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
            <div style={{height:2,width:"100%",borderRadius:1,background:d||a?T.gold:T.border,transition:"background 0.3s"}}/>
            <span style={{fontSize:8,letterSpacing:"0.06em",textTransform:"uppercase",textAlign:"center",color:a?T.gold:d?T.goldDim:T.textDim,lineHeight:1.3}}>{s}</span>
          </div>
        );})}
      </div>
      ...
      {/* Step 0 */}
      {step===0&&(<Card>
        <CardTitle>{L==="vi"?"Thuế Năm Hiện Tại":L==="es"?"Impuesto Año Actual":"Current Year Tax"}</CardTitle>
        <CardSub>{L==="vi"?"Tùy chọn — chỉ cần cho quy tắc an toàn 90%":L==="es"?"Opcional — solo para el puerto seguro del 90%":"Optional — needed only for the 90% safe harbor check"}</CardSub>
        <Fld label={TX.s0q}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
            {[{v:true,l:TX.yes},{v:false,l:TX.est},{v:"skip",l:TX.skip}].map(o=>(
              <button key={String(o.v)} onClick={()=>s("kt",o.v)}
                style={{padding:"10px 6px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",fontSize:11,lineHeight:1.4,textAlign:"center",transition:"all 0.15s",
                  background:inp.kt===o.v?"rgba(200,169,110,0.12)":T.bgCard,
                  border:`1px solid ${inp.kt===o.v?T.gold:T.border}`,
                  color:inp.kt===o.v?T.gold:T.textMid}}>
                {o.l}
              </button>
            ))}
          </div>
        </Fld>
        {inp.kt===true&&<Fld label={TX.taxL} hint={TX.taxH}><Inp value={inp.tax} onChange={v=>s("tax",v)} placeholder="0" prefix="$"/></Fld>}
        {inp.kt===false&&(<>
          <Fld label={TX.fsL}><Sel value={inp.fs} onChange={v=>s("fs",v)} options={FS}/></Fld>
          <Fld label={TX.incL} hint={TX.incH}><Inp value={inp.gross} onChange={v=>s("gross",v)} placeholder="0" prefix="$"/></Fld>
          {parse(inp.gross)>0&&<IBox tone="green">≈ {fmt(estTax(parse(inp.gross),inp.fs))} estimated tax</IBox>}
        </>)}
        {inp.kt==="skip"&&<IBox tone="amber">{TX.skipInfo}</IBox>}
        {inp.kt!==null&&(<><Divider/>
      ...
      {/* Step 1 */}
      {step===1&&(<Card>
        <CardTitle>{L==="vi"?"Thuế Năm Trước":L==="es"?"Impuesto Año Anterior":"Prior Year Tax"}</CardTitle>
        <CardSub>{L==="vi"?"Dùng để tính ngưỡng an toàn":L==="es"?"Para calcular el umbral de puerto seguro":"Used to calculate safe harbor threshold"}</CardSub>
        <Fld label={TX.priorL} hint={TX.priorH}><Inp value={inp.prior} onChange={v=>s("prior",v)} placeholder="0" prefix="$"/></Fld>
        <Fld label={TX.agiL} hint={TX.agiH}><Inp value={inp.agi} onChange={v=>s("agi",v)} placeholder="0" prefix="$"/></Fld>
        {parse(inp.agi)>0&&parse(inp.prior)>0&&(
          <IBox tone={priorAGI>thr?undefined:"green"}>
            {priorAGI>thr
              ?<>AGI {L==="vi"?"vượt quá":L==="es"?"superó":"exceeded"} <strong>${thr.toLocaleString()}</strong> — {L==="vi"?"phải nộp":L==="es"?"debe pagar":"must pay"} <strong>110%</strong> {L==="vi"?"thuế năm trước":L==="es"?"del impuesto anterior":"of prior year tax"} (<strong>{fmt(parse(inp.prior)*1.10)}</strong>).</>
              :<>AGI {L==="vi"?"dưới":L==="es"?"bajo":"under"} <strong>${thr.toLocaleString()}</strong> — {L==="vi"?"ngưỡng an toàn yêu cầu":L==="es"?"puerto seguro requiere":"safe harbor requires"} <strong>100%</strong> {L==="vi"?"thuế năm trước":L==="es"?"del impuesto anterior":"of prior year tax"} (<strong>{fmt(parse(inp.prior))}</strong>).</>
            }
          </IBox>
        )}
        <NavRow><Btn ghost onClick={()=>setStep(0)}>{TX.back}</Btn><Btn onClick={()=>setStep(2)} disabled={!inp.prior||!inp.agi}>{TX.next}</Btn></NavRow>
      </Card>)}

      {/* Step 2 */}
      {step===2&&(<Card>
        <CardTitle>{L==="vi"?"Thanh Toán Hàng Quý":L==="es"?"Pagos Trimestrales":"Quarterly Payments"}</CardTitle>
        <CardSub>{TX.s2sub}</CardSub>
        {QQ.map((q,i)=>(<Fld key={i} label={`${q.l} · ${q.mo} · ${q.due}`}><Inp value={inp[`q${i+1}`]} onChange={v=>s(`q${i+1}`,v)} placeholder="0" prefix="$"/></Fld>))}
        <p style={{fontSize:11,color:T.textDim,margin:"0 0 14px",fontStyle:"italic"}}>{TX.s2zero}</p>
        <Divider/>
        <Toggle value={useAI} onChange={setAI} label={TX.aiToggle} desc={TX.aiDesc}/>
        <NavRow>
          <Btn ghost onClick={()=>setStep(1)}>{TX.back}</Btn>
          <Btn onClick={()=>useAI?setStep(3):doCalc()} accent={useAI}>{useAI?TX.nextAI:TX.calc}</Btn>
        </NavRow>
      </Card>)}

      {/* Step 3 AI */}
      {step===3&&useAI&&(<Card accent>
        <CardTitle accent>{L==="vi"?"Lịch AI — Thu Nhập Theo Kỳ":L==="es"?"Horario AI — Por Período":"Schedule AI — Income by Period"}</CardTitle>
        <CardSub>{TX.s3sub}</CardSub>
        <IBox tone="purple">{L==="vi"?"Nhập tổng lũy kế từ 1/1 đến cuối mỗi kỳ.":L==="es"?"Ingrese totales acumulados desde el 1 de enero hasta el final de cada período.":"Enter cumulative totals from Jan 1 through end of each period."}</IBox>
        <div style={{marginTop:14}}>
          {[["Jan–Mar","T1–3","Ene–Mar"],[" Jan–May","T1–5","Ene–May"],["Jan–Aug","T1–8","Ene–Ago"],["Jan–Dec","T1–12","Ene–Dic"]].map((p,i)=>(
            <div key={i} style={{marginBottom:12,padding:"12px 14px",background:"#0a0810",border:"1px solid #2a1840",borderRadius:8}}>
              <div style={{fontSize:10,color:"#7050a0",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:10}}>
                Period {i+1} · {L==="vi"?p[1]:L==="es"?p[2]:p[0]} · ×{AI_FACTORS[i]}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <div>
                  <label style={{fontSize:10,color:"#5a4070",display:"block",marginBottom:4,letterSpacing:"0.06em",textTransform:"uppercase"}}>{TX.aiIncL}</label>
                  <Inp value={inp[`a${i+1}`]} onChange={v=>s(`a${i+1}`,v)} placeholder="0" prefix="$"/>
                </div>
                <div>
                  <label style={{fontSize:10,color:"#5a4070",display:"block",marginBottom:4,letterSpacing:"0.06em",textTransform:"uppercase"}}>{TX.aiWhL}</label>
                  <Inp value={inp[`aw${i+1}`]} onChange={v=>s(`aw${i+1}`,v)} placeholder="0" prefix="$"/>
                </div>
              </div>
              {parse(inp[`a${i+1}`])>0&&<div style={{marginTop:7,fontSize:11,color:"#6a40a0"}}>
                Ann: {fmt(parse(inp[`a${i+1}`])*AI_FACTORS[i])} · Tax: {fmt(estTax(parse(inp[`a${i+1}`])*AI_FACTORS[i],inp.fs))}
              </div>}
            </div>
          ))}
        </div>
        <NavRow>
          ...
          <Btn accent onClick={doCalc} disabled={!inp.a1||!inp.a2||!inp.a3||!inp.a4}>{TX.calc}</Btn>
        </NavRow>
      </Card>)}

      {/* Results */}
      {step===steps.length-1&&res&&(<div>
        <div style={{background:res.met?T.greenBg:T.redBg,border:`1px solid ${res.met?"#1a4020":"#3a1a0a"}`,borderRadius:12,padding:"26px 20px",textAlign:"center",marginBottom:14}}>
          {res.useAI&&<div style={{fontSize:9,letterSpacing:"0.3em",textTransform:"uppercase",marginBottom:6,color:"#5020a0"}}>{TX.rAI}</div>}
          <div style={{fontSize:9,letterSpacing:"0.3em",textTransform:"uppercase",marginBottom:8,color:res.met?"#2a6030":"#8a5020"}}>{res.met?TX.rMet:TX.rPen}</div>
          <div style={{fontSize:"clamp(40px,8vw,58px)",fontWeight:300,lineHeight:1,color:res.met?T.green:T.gold,letterSpacing:"-0.03em"}}>{res.met?"$0":fmt(res.totalPenalty)}</div>
          {res.est&&<p style={{color:T.textDim,marginTop:6,fontSize:10}}>{L==="vi"?"Dựa trên thuế ước tính":L==="es"?"Basado en impuesto estimado":"Based on estimated tax of"} {fmt(res.effTax)}</p>}
          {!res.effTax&&<p style={{color:T.textDim,marginTop:6,fontSize:10}}>{L==="vi"?"Sử dụng ngưỡng an toàn năm trước":L==="es"?"Usando solo año anterior":"Using prior year safe harbor only"}</p>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
          {[{l:TX.rPaid,v:fmt(res.totalPaid)},{l:TX.rSH,v:fmt(res.shMin)},{l:TX.rShort,v:fmt(res.shortfall),hi:res.shortfall>0},{l:TX.rPenLbl,v:fmt(res.totalPenalty),hi:res.totalPenalty>0}].map((it,i)=>(
            <div key={i} style={{background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:8,padding:"11px 14px"}}>
              <div style={{fontSize:9,color:T.textDim,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:3}}>{it.l}</div>
              <div style={{fontSize:18,color:it.hi?T.red:T.text}}>{it.v}</div>
            </div>
          ))}
        </div>
        {!res.useAI&&(<Card>
          <CardTitle>{TX.rSafeTitle}</CardTitle>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginTop:8}}>
            <tbody>
              <tr style={{borderBottom:`1px solid ${T.border}`}}>
                <td style={{padding:"8px 0",color:T.textMid}}>{res.shPct===1.10?"110%":"100%"} {L==="vi"?"thuế năm trước":L==="es"?"impuesto año anterior":"of prior year tax"}</td>
                <td style={{padding:"8px 0",textAlign:"right",color:T.text}}>{fmt(res.sp)}</td>
              </tr>
              {res.effTax?(
                <tr style={{borderBottom:`1px solid ${T.border}`}}>
                  <td style={{padding:"8px 0",color:T.textMid}}>90% {L==="vi"?"thuế năm nay":L==="es"?"impuesto año actual":"of current year tax"}{res.est?` (${L==="vi"?"ước tính":L==="es"?"est":"est"})`:""}</td>
                  <td style={{padding:"8px 0",textAlign:"right",color:T.text}}>{fmt(res.sc)}</td>
                </tr>
              ):(
                <tr style={{borderBottom:`1px solid ${T.border}`}}>
                  <td style={{padding:"8px 0",color:T.textDim,fontStyle:"italic"}}>90% {L==="vi"?"thuế năm nay":L==="es"?"impuesto año actual":"of current year tax"}</td>
                  <td style={{padding:"8px 0",textAlign:"right",color:T.textDim,fontStyle:"italic"}}>{TX.rNP}</td>
                </tr>
              )}
              <tr>
                <td style={{padding:"10px 0 0",color:T.gold,fontWeight:600}}>{TX.rUsed} <span style={{fontWeight:400,fontSize:11,color:T.textDim}}>{TX.rLesser}</span></td>
                <td style={{padding:"10px 0 0",textAlign:"right",color:T.gold,fontWeight:600}}>{fmt(res.shMin)}</td>
              </tr>
            </tbody>
          </table>
          {res.effTax&&<IBox tone={res.win==="current"?"green":undefined}>
            {res.win==="current"
              ?<>{L==="vi"?"Số tiền 90% năm nay thấp hơn":L==="es"?"El monto del 90% actual es menor":"The 90% current-year amount is lower"} — {L==="vi"?"ngưỡng dễ đạt hơn":L==="es"?"umbral más fácil de alcanzar":"easier threshold to meet"}.</>
              :<>{L==="vi"?"Số tiền":L==="es"?"El monto del":"The"} {res.shPct===1.10?"110%":"100%"} {L==="vi"?"năm trước thấp hơn":L==="es"?"año anterior es menor":"prior-year amount is lower"} — {L==="vi"?"đó là ngưỡng được sử dụng":L==="es"?"ese es el umbral utilizado":"that's the threshold used"}.</>
            }
          </IBox>}
        </Card>)}
        {res.useAI&&(<Card accent>
          <CardTitle accent>{TX.rAITitle}</CardTitle>
          <div style={{overflowX:"auto",marginTop:8}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
              <thead><tr style={{borderBottom:`1px solid #2a1840`}}>
                {["Qtr","Ann Inc","Est Tax","AI Req","Prior","Used","Short","Penalty"].map(h=>(
                  <th key={h} style={{textAlign:"left",padding:"6px 4px 6px 0",color:"#5a4070",fontWeight:400,fontSize:9,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {res.qd.map((q,i)=>(
                  <tr key={i} style={{borderBottom:`1px solid #140a1f`}}>
                    <td style={{padding:"8px 4px 8px 0",color:"#9060c0"}}>{q.l}</td>
                    <td style={{padding:"8px 4px 8px 0",color:"#604080"}}>{fmt(q.aInc)}</td>
                    <td style={{padding:"8px 4px 8px 0",color:"#604080"}}>{fmt(q.aTax)}</td>
                    <td style={{padding:"8px 4px 8px 0",color:"#7050a0"}}>{fmt(q.cr)}</td>
                    <td style={{padding:"8px 4px 8px 0",color:"#7050a0"}}>{fmt(q.pr)}</td>
                    <td style={{padding:"8px 4px 8px 0",color:"#b080e0",fontWeight:600}}>{fmt(q.er)}</td>
                    <td style={{padding:"8px 4px 8px 0",color:q.shortfall>0?T.red:T.green}}>{fmt(q.shortfall)}</td>
                    <td style={{padding:"8px 4px 8px 0",color:q.penalty>0?T.gold:T.green}}>{fmt(q.penalty)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {!res.met&&!res.useAI&&(<Card>
          <CardTitle>{TX.rQTitle}</CardTitle>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginTop:8}}>
            <thead><tr style={{borderBottom:`1px solid ${T.border}`}}>
              {[L==="vi"?"Quý":L==="es"?"Trim":"Qtr",L==="vi"?"Đáo Hạn":L==="es"?"Fecha":"Due",L==="vi"?"Yêu Cầu":L==="es"?"Req":"Required",L==="vi"?"Thiếu":L==="es"?"Déficit":"Shortfall",L==="vi"?"Phạt":L==="es"?"Multa":"Penalty"].map(h=>(
                <th key={h} style={{textAlign:"left",padding:"6px 0",color:T.textDim,fontWeight:400,fontSize:10,letterSpacing:"0.08em",textTransform:"uppercase"}}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {res.qd.map((q,i)=>(
                <tr key={i} style={{borderBottom:`1px solid ${T.border}`}}>
                  <td style={{padding:"8px 0",color:T.gold}}>{q.l}</td>
                  <td style={{padding:"8px 0",color:T.textMid}}>{q.due}</td>
                  <td style={{padding:"8px 0",color:T.textMid}}>{fmt(q.required)}</td>
                  <td style={{padding:"8px 0",color:q.shortfall>0?T.red:T.green}}>{fmt(q.shortfall)}</td>
                  <td style={{padding:"8px 0",color:q.penalty>0?T.gold:T.green}}>{fmt(q.penalty)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Card>
          <CardTitle>{TX.rTips}</CardTitle>
          <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>
            {[
              res.useAI?(L==="vi"?"Khớp thanh toán mỗi quý với thu nhập thực tế":L==="es"?"Ajuste el pago de cada trimestre a sus ingresos reales":`Pay at least ${fmt(res.shMin/4)}/quarter via annualized method`):`Pay at least ${fmt(res.shMin/4)}/quarter`,
              L==="vi"?"Nộp tại IRS Direct Pay (irs.gov/payments)":L==="es"?"Pague en IRS Direct Pay (irs.gov/payments)":"Set up payments at IRS Direct Pay (irs.gov/payments)",
              L==="vi"?"Tăng khấu trừ W-4 — tính là nộp đều cả năm":L==="es"?"Aumente retención W-4 — cuenta como pagado uniformemente":"Increase W-4 withholding — counts as paid evenly all year",
              L==="vi"?"Ngày đáo hạn: 15/4 · 16/6 · 15/9 · 15/1":L==="es"?"Fechas: 15 abr · 16 jun · 15 sep · 15 ene":"Deadlines: Apr 15 · Jun 16 · Sep 15 · Jan 15",
            ].map((tip,i)=>(
              <div key={i} style={{display:"flex",gap:8,alignItems:"flex-start"}}>
                <span style={{color:T.gold,fontSize:9,marginTop:4,flexShrink:0}}>◆</span>
                <span style={{color:T.textMid,fontSize:12,lineHeight:1.6}}>{tip}</span>
              </div>
            ))}
          </div>
        </Card>
        ...
        <div style={{textAlign:"center",marginTop:12}}>
          <Btn ghost small onClick={()=>{setStep(0);setRes(null);setAI(false);setInp({kt:null,fs:"single",gross:"",tax:"",wh:"",prior:"",agi:"",q1:"",q2:"",q3:"",q4:"",a1:"",a2:"",a3:"",a4:"",aw1:"",aw2:"",aw3:"",aw4:""});}}>{TX.again}</Btn>
        </div>
      </div>)}
    </div>
  );
