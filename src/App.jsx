...
function DepreciationCalc({lang:L="en"}){
  ...
  async function handlePDF(e){
    ...
    try{
      ...
      const prompt=`You are a CPA analyzing a real estate closing statement. Extract every line item and categorize per IRS rules. Return ONLY valid JSON:
{"documentType":"HUD-1 or CD","hasloan":true,"purchasePrice":null,
"sectionB":{"titleInsuranceLender":null,"titleInsuranceOwner":null,"titleSearch":null,"otherTitleFees":null,"settlementFee":null,"recordingCharges":null,"taxStamps":null,"transferTaxes":null,"attorneyFeesBasis":null,"surveyFee":null,"inspections":null,"appraisalBasis":null,"otherBasis":null},
"sectionC":{"originationFee":null,"discountPoints":null,"appraisalLender":null,"creditReport":null,"mortgageInsurancePMI":null,"assumptionFee":null,"underwritingFee":null,"attorneyFeeLoan":null,"lenderOther":null,"lenderCredit":null},
"sectionD":{"propertyTaxClosing":null,"prepaidInterest":null,"insuranceMIP":null,"proratedRent":null},
"sectionE":{"escrowInsurance":null,"escrowTax":null,"escrowMortgageIns":null,"aggregateAdj":null},
"sectionF":{"earnestMoney":null,"loanFunds":null,"sellerCredit":null,"taxAdjSeller":null,"optionFee":null,"proratedHOA":null},
"poc":{"pocAppraisal":null},
"flags":["short unusual item note — max 5"]}`;
      ...
      const b=d.sectionB||{};
      Object.entries(b).forEach(([k,v])=>{if(v)s(k,String(v));});
      const c=d.sectionC||{};
      Object.entries(c).forEach(([k,v])=>{if(v)s(k,String(v));});
      const dd=d.sectionD||{};
      Object.entries(dd).forEach(([k,v])=>{if(v)s(k,String(v));});
      const e=d.sectionE||{};
      Object.entries(e).forEach(([k,v])=>{if(v)s(k,String(v));});
      const f=d.sectionF||{};
      Object.entries(f).forEach(([k,v])=>{if(v)s(k,String(v));});
      const poc=d.poc||{};
      if(poc.pocAppraisal)s("pocAppraisal",String(poc.pocAppraisal));
      setPdfFlags(d.flags||[]);
    }catch(err){setPdfFlags(["Could not read PDF — please enter manually."]);}
    setPdfLoading(false);
  }
  ...
  function MF({label,field,hint,credit,income}){
    const isCredit=credit||income;
    return(
      <div style={{marginBottom:6}}>
        <div style={{fontSize:10,color:isCredit?T.amber:T.textMid,marginBottom:2,
          fontWeight:isCredit?600:400}}>
          {label}{credit&&<span style={{fontSize:9,color:T.amber,marginLeft:4}}>← reduces basis</span>}
          {income&&<span style={{fontSize:9,color:T.red,marginLeft:4}}>← taxable income</span>}
        </div>
        {hint&&<div style={{fontSize:9,color:T.textDim,marginBottom:2,fontStyle:"italic"}}>{hint}</div>}
    ...
    <div>
      ...
      {step===0&&(<div>
        ...
        <SB color="#2a5a3a" tc="#90d4a0" title="A — Purchase Price">
          <MF label={"Purchase price (from sales contract / HUD-1 Line 101)"} field="purchasePrice"/>
        </SB>

        {/* B — Added to Basis */}
        <SB color="#2a3a6a" tc="#90b0f0" title="B — HUD-1 Expenses Added to Property Cost Basis (capitalized — NOT immediately deductible)">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Title Insurance — Lender's Policy"} field="titleInsuranceLender" hint="HUD-1 line 1101"/>
            <MF label={"Title Insurance — Owner's Policy"} field="titleInsuranceOwner" hint="HUD-1 line 1103"/>
            <MF label={"Title Search / Abstract / Title Exam"} field="titleSearch" hint="HUD-1 line 1102"/>
            <MF label={"Other Title Fees"} field="otherTitleFees" hint="Endorsements, binder, etc."/>
            <MF label={"Settlement / Closing Fee"} field="settlementFee" hint="HUD-1 line 1100"/>
            <MF label={"Gov't Recording Charges"} field="recordingCharges" hint="HUD-1 line 1201"/>
            <MF label={"State/City/County Tax Stamps"} field="taxStamps" hint="HUD-1 line 1202"/>
            <MF label={"Transfer Taxes"} field="transferTaxes" hint="HUD-1 line 1203"/>
            <MF label={"Attorney Fees (title-related, not loan)"} field="attorneyFeesBasis"/>
            <MF label={"Survey Fee"} field="surveyFee" hint="HUD-1 line 1301"/>
            <MF label={"Inspections (not required by lender)"} field="inspections" hint="HUD-1 lines 1302–1305"/>
            <MF label={"Appraisal (NOT required by lender)"} field="appraisalBasis" hint="Buyer's choice — adds to basis"/>
            <MF label={"Other Closing Costs"} field="otherBasis" hint="Home warranty, buyer agent commission, etc."/>
          </div>
        ...
        <SB color="#4a4a1a" tc="#d4c840" title="Paid Outside Closing (POC) — add to Loan Cost or Basis">
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Appraisal (required by lender — paid POC)"} field="pocAppraisal" hint="Usually goes to Loan Cost"/>
            <MF label={"Inspection (paid outside closing)"} field="pocInspection" hint="Goes to Property Basis"/>
            <MF label={"Other POC item"} field="pocOther" hint="Goes to Property Basis"/>
          </div>
        ...
        {hasloan&&<SB color="#5a3a6a" tc="#c090d8" title="C — HUD-1 Expenses — Loan Cost Basis (amortize over loan term)">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Loan Origination Fee"} field="originationFee" hint="HUD-1 line 801"/>
            <MF label={"Loan Discount / Points"} field="discountPoints" hint="HUD-1 line 802"/>
            <MF label={"Appraisal (Required by Lender)"} field="appraisalLender" hint="On statement only — not POC"/>
            <MF label={"Credit Report"} field="creditReport" hint="HUD-1 line 804"/>
            <MF label={"Mortgage Insurance PMI (financed into loan)"} field="mortgageInsurancePMI" hint="Not upfront MIP — see Section D"/>
            <MF label={"Assumption / Application Fee"} field="assumptionFee"/>
            <MF label={"Underwriting Fee"} field="underwritingFee" hint="HUD-1 line 808"/>
            <MF label={"Attorney Fees (under loan section)"} field="attorneyFeeLoan" hint="Loan-related attorney work"/>
          ...
          <div style={{borderTop:"1px solid "+T.border,marginTop:10,paddingTop:10}}>
            <MF label={"Lender Credit to Buyer → Enter as POSITIVE"} field="lenderCredit"
              hint="Reduces loan cost basis and reduces cash to close"/>
          </div>
        ...
        <SB color="#1a4a3a" tc="#70d0a0" title="D — HUD-1 Expenses Currently Deductible (deduct in year of closing)">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Property Taxes Paid at Closing ← DEDUCTIBLE"} field="propertyTaxClosing" hint="Schedule E line 16"/>
            <MF label={"Prepaid Mortgage Interest ← DEDUCTIBLE"} field="prepaidInterest" hint="Closing date to month-end · Schedule E"/>
            <MF label={"Homeowners Insurance + Upfront MIP/PMI ← DEDUCTIBLE"} field="insuranceMIP" hint="Full premium deductible (cash basis · IRS Rev. Rul. 2007-3)"/>
            <MF label={"Prorated Rent from Seller → TAXABLE INCOME"} field="proratedRent" hint="Report on Schedule E · enter positive" income/>
          </div>
        ...
        <SB color="#3a2a5a" tc="#a080c8" title="E — Reserves and Deposits (escrow — deductible only when paid out)">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Deposit with Lender — Homeowners Insurance"} field="escrowInsurance"/>
            <MF label={"Deposit with Lender — Property Tax"} field="escrowTax"/>
            <MF label={"Deposit with Lender — Mortgage Insurance"} field="escrowMortgageIns"/>
            <MF label={"Aggregate Adjustment Credit → Enter as POSITIVE"} field="aggregateAdj"
              hint="Lender credit to prevent escrow overfunding — reduces cash to close"/>
          </div>
        </SB>
        ...
        <SB color="#5a2a1a" tc="#e09060" title="F — Reductions to Amount Due (all enter as POSITIVE)">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <MF label={"Earnest Money Deposit"} field="earnestMoney" hint="Cash paid by buyer — no basis effect"/>
            <MF label={"Loan Funds (Mortgage Amount)"} field="loanFunds" hint="From lender"/>
            <MF label={"Seller Credit / Concession"} field="sellerCredit" hint="Reduces basis" credit/>
            <MF label={"Tax Adjustment Unpaid by Seller"} field="taxAdjSeller" hint="Reduces basis + reduces cash to close" credit/>
            <MF label={"Option Fee / Option Period Credit"} field="optionFee" hint="Buyer's own cash — reduces cash to close only"/>
            <MF label={"Prorated HOA / Other Credits"} field="proratedHOA" hint="Reduces basis" credit/>
          </div>
        </SB>
