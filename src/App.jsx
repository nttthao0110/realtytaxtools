...
function DepreciationCalc({lang:L="en"}){
  ...
  // ── Simple input row ─────────────────────────────────────────
  function Row({label,field,hint,credit,income}){
    ...
    return(
      <div style={{marginBottom:10}}>
        ...
        <div style={{position:"relative"}}>
          ...
          <input type="text" inputMode="decimal" value={inp[field]}
            onChange={e=>s(field,e.target.value)} placeholder="0"
    ...
    return(
      <div>
        ...
        <p style={{fontSize:9,color:T.textDim,textAlign:"center",margin:"4px 0 10px"}}>
          {"Estimate only — 27.5yr straight-line. For full HUD-1 analysis with all asset classes, use Detailed Mode."}
        </p>
        <button onClick={()=>setMode("detail")}
          style={{width:"100%",padding:"10px",borderRadius:7,cursor:"pointer",fontFamily:"inherit",
            background:"rgba(200,169,110,0.08)",border:"1px solid "+T.goldDim,color:T.gold,fontSize:12}}>
          {"Switch to Detailed Mode — Full HUD-1 Breakdown →"}
        </button>
      </div>
    );
  }
          ...
          {step===0&&(
            <div>
              ...
              <DeprecSection title="A — Purchase Price" color="#2a5a3a" tc="#90d4a0" open={true}>
                <Row label="Purchase price (from sales contract / HUD-1 Line 101)" field="purchasePrice"/>
              </DeprecSection>
              ...
              <DeprecSection title="B — HUD-1 Expenses Added to Property Cost Basis" color="#2a3a6a" tc="#90b0f0">
                ...
                <Row label="Title Insurance — Lender's Policy" field="titleInsuranceLender" hint="HUD-1 line 1101"/>
                <Row label="Title Insurance — Owner's Policy" field="titleInsuranceOwner" hint="HUD-1 line 1103"/>
                <Row label="Title Search / Abstract / Title Exam" field="titleSearch"/>
                <Row label="Other Title Fees (endorsement, courier, guaranty fee, binder)" field="otherTitleFees"/>
                <Row label="Settlement / Closing Fee" field="settlementFee" hint="HUD-1 line 1100"/>
                <Row label="Gov't Recording Charges" field="recordingCharges" hint="HUD-1 line 1201"/>
                <Row label="Tax Stamps / Transfer Taxes" field="taxStamps"/>
                <Row label="Transfer Taxes" field="transferTaxes"/>
                <Row label="Attorney Fees — Title/Closing Work Only" field="attorneyFeesBasis" hint="Title exam, deed prep, closing. NOT loan-related attorney fees."/>
                <Row label="Survey Fee" field="surveyFee"/>
                <Row label="Inspections (buyer's choice, not lender-required)" field="inspections"/>
                <Row label="Appraisal — Buyer's Own (not required by lender)" field="appraisalBasis"/>
                <Row label="Other Closing Costs (home warranty, buyer agent commission)" field="otherBasis"/>
              ...
              <DeprecSection title="Paid Outside Closing (POC)" color="#4a4a1a" tc="#d4c840">
                ...
                <Row label="Appraisal paid outside closing (POC)" field="pocAppraisal"/>
                <div style={{marginBottom:10}}>
                ...
                <Row label="Inspection (paid outside closing) → Basis" field="pocInspection"/>
                <Row label="Other POC item → Basis" field="pocOther"/>
              </DeprecSection>
              ...
              {hasloan&&(
                <DeprecSection title="C — Loan Costs (amortize over loan term)" color="#5a3a6a" tc="#c090d8">
                  ...
                  <Row label="Loan Origination Fee" field="originationFee"/>
                  <Row label="Loan Discount / Points" field="discountPoints"/>
                  <Row label="Appraisal — Required by Lender (on statement, not POC)" field="appraisalLender"/>
                  <Row label="Credit Report" field="creditReport"/>
                  <Row label="Underwriting Fee / Processing Fee" field="underwritingFee"/>
                  <Row label="Attorney Fees — Loan Section Only" field="attorneyFeeLoan" hint="Only if listed under loan costs in HUD-1"/>
                  <Row label="Other Lender Fees (flood cert, tax service, wire fee)" field="lenderOther"/>
                  <div style={{borderTop:"1px solid "+T.border,marginTop:8,paddingTop:8}}>
                    <Row label="Lender Credit → Enter as POSITIVE (reduces loan cost)" field="lenderCredit" credit/>
                  </div>
                  {grossLoanCosts>0&&<div style={{marginTop:6,padding:"5px 10px",background:"rgba(160,128,192,0.1)",borderRadius:4,fontSize:11,color:"#b080d8"}}>
                    Gross loan costs: <strong>{fmt(grossLoanCosts)}</strong>
                    {lenderCreditAmt>0&&<> · Credit: <strong>({fmt(lenderCreditAmt)})</strong></>}
                    {" · "}Net: <strong>{fmt(netLoanCosts)}</strong>
                  </div>}
                </DeprecSection>
              )}
              ...
              <DeprecSection title="D — Currently Deductible This Year" color="#1a4a3a" tc="#70d0a0">
                {/* Property tax selector */}
                <div style={{marginBottom:12,padding:"10px",background:"rgba(192,112,80,0.06)",border:"1px solid rgba(192,112,80,0.2)",borderRadius:6}}>
                  <div style={{fontSize:11,color:"#c07050",fontWeight:600,marginBottom:6}}>{"Property Tax at Closing — enter in ONE place only:"}</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                    {[{v:"pay",l:"I PAY taxes",d:"You pay buyer's share → deductible"},{v:"credit",l:"Seller CREDITS me",d:"Seller's unpaid taxes → reduces basis"}].map(o=>(
                      <button key={o.v} onClick={()=>{s("taxProrationType",o.v);s("propertyTaxClosing","");s("taxAdjSeller","");}}
                        style={{padding:"7px",borderRadius:6,cursor:"pointer",fontFamily:"inherit",textAlign:"left",
                        ...
                        <div style={{fontSize:9,color:T.textDim,marginTop:1}}>{o.d}</div>
                      </button>
                    ))}
                  </div>
                  {inp.taxProrationType==="pay"&&(
                    <Row label="Property Taxes Paid at Closing (deductible)" field="propertyTaxClosing"/>
                  )}
                  {inp.taxProrationType==="credit"&&(
                    <Row label="Tax Proration Credit FROM Seller (reduces basis)" field="taxAdjSeller" credit/>
                  )}
                </div>
                <Row label="Prepaid Mortgage Interest (deductible)" field="prepaidInterest" hint="Closing date to month-end · Schedule E"/>
                <Row label="Homeowners Insurance + Upfront MIP/PMI (deductible)" field="insuranceMIP"/>
                <Row label="Prorated Rent FROM Seller (taxable income to you)" field="proratedRent" income/>
              </DeprecSection>

              {/* E — Escrow */}
              <DeprecSection title="E — Reserves and Escrow Deposits" color="#3a2a5a" tc="#a080c8">
                <Row label="Homeowners Insurance Escrow" field="escrowInsurance"/>
                <Row label="Property Tax Escrow" field="escrowTax"/>
                <Row label="Mortgage Insurance Escrow" field="escrowMortgageIns"/>
                <Row label="Aggregate Adjustment Credit → Enter as POSITIVE" field="aggregateAdj" credit/>
              </DeprecSection>

              {/* F — Reductions */}
              <DeprecSection title="F — Reductions to Amount Due (all POSITIVE)" color="#5a2a1a" tc="#e09060">
                <Row label="Earnest Money Deposit" field="earnestMoney" hint="Cash paid by buyer — no basis effect"/>
                <Row label="Loan Funds (Mortgage Amount)" field="loanFunds"/>
                <Row label="Seller Credit / Concession" field="sellerCredit" credit/>
                {inp.taxProrationType==="credit"&&inp.taxAdjSeller&&(
                  <div style={{padding:"6px 10px",background:"rgba(200,169,110,0.06)",borderRadius:4,fontSize:11,color:T.amber,marginBottom:8}}>
                    {"Tax proration credit "+fmt(parse(inp.taxAdjSeller))+" already entered in Section D above"}
                  </div>
                )}
                <Row label="Option Fee (reduces cash-to-close only, not basis)" field="optionFee"/>
                <Row label="Prorated HOA / Other Credits" field="proratedHOA" credit/>
              </DeprecSection>

              {/* Live reconciliation */}
              {parse(inp.purchasePrice)>0&&(
                <div style={{background:"linear-gradient(135deg,#0a1a0e,#080e0a)",border:"1px solid "+T.border,
                  borderRadius:10,padding:"12px 14px",marginBottom:12}}>
                  <p style={{fontSize:10,color:T.green,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",margin:"0 0 8px"}}>
