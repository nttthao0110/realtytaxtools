...
function STRCalc({lang:L="en"}){
  const [inp,setInp]=useState({rentalDays:"",personalDays:"",income:"",expenses:"",fairRentDays:"",fs:"single",materialParticipation:false});
  const [res,setRes]=useState(null);
  const s=(k,v)=>setInp(p=>({...p,[k]:v}));
  const TX={
  ...
  function doCalc(){
    ...
    const netIncome=inc-deductibleExp;
    // 14-day rule: if rented ≤14 days, income NOT reportable (§280A(g))
    const rule14=rd<=14&&rd>0;
    // Vacation home test
    const personalThreshold=Math.max(14,Math.round(rd*0.10));
    const isVacationHome=pd>personalThreshold;
    const isMaterialParticipation=inp.materialParticipation;
    // SE tax only if providing substantial services (hotel-like) — rare for STR
    const seTax=0;
    // Loss treatment:
    // - Material participation + STR (avg stay ≤7 days) = NON-PASSIVE, can offset ordinary income
    // - Passive (most cases) = PAL rules, $25k if AGI<$100k with active participation
    // - Vacation home = limited to income
    const isNonPassive=isMaterialParticipation&&!isVacationHome;
    const estimatedIncomeTax=netIncome>0?Math.round(netIncome*0.22):0;
    setRes({rd,pd,inc,exp,rentalPct,deductibleExp,netIncome,
      rule14,isVacationHome,isNonPassive,isMaterialParticipation,
      seTax,estimatedIncomeTax,total,personalThreshold});
  }
  return(
    <div>
      ...
      <div style={{padding:"10px 14px",background:T.bgCard2,border:"1px solid "+T.border,borderRadius:8,marginBottom:8}}>
        <Toggle value={inp.materialParticipation} onChange={v=>s("materialParticipation",v)}
          label={"I materially participate in this rental"}
          desc={"500+ hrs/yr or >50% of work time. If yes AND avg stay ≤7 days, losses can offset ordinary income (non-passive)."}/>
      </div>
      <div style={{display:"flex",gap:8,marginTop:4}}>
        <GoldBtn onClick={doCalc}>{TX.calc}</GoldBtn>
        <OutlineBtn onClick={()=>{setInp({rentalDays:"",personalDays:"",income:"",expenses:"",fairRentDays:"",fs:"single",materialParticipation:false});setRes(null);}}>{TX.reset}</OutlineBtn>
      </div>
      {res&&(<div style={{marginTop:20}}>
        ...
        {!res.rule14&&(<>
          ...
          <IBox tone={res.isVacationHome?"amber":undefined}>
            {res.isVacationHome
              ?<>{"⚠ Vacation home — personal use exceeds threshold. Expenses capped at rental income. Losses cannot offset other income. Report on Schedule E."}</>
              :res.isNonPassive
              ?<>{"✅ NON-PASSIVE activity — material participation + short-term rental. Net losses can offset ordinary income without PAL limitations. Report on Schedule E (not Schedule C — no SE tax)."}</>
              :<>{"✓ Passive rental activity (Schedule E). No self-employment tax. Net losses subject to PAL rules — up to $25k deductible against ordinary income if AGI under $100k with active participation."}</>
            }
          </IBox>
        </>)}
