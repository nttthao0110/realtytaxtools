...
function DepreciationCalc({lang:L="en"}){
  ...
  function Row({label,field,hint,credit,income}){
    // Uses inp[field] and s() from DepreciationCalc closure
    // Row has NO hooks so this is safe — not a hook violation
    const isC=credit||income;
    const val=inp[field];
    const onChange=e=>s(field,e.target.value);
    return(
      <div style={{marginBottom:10}}>
          ...
          {step===0&&(
            <div>
              ...
              <DeprecSection title="D — Currently Deductible This Year" color="#1a4a3a" tc="#70d0a0">
                <Row label={"Property Taxes Paid at Closing (deductible)"} field="propertyTaxClosing" hint="Only if YOU pay taxes at closing as a charge. If seller gives you a credit for their unpaid taxes, enter that amount in Section F below."/>
              ...
              <DeprecSection title="F — Reductions to Amount Due (all POSITIVE)" color="#5a2a1a" tc="#e09060">
                ...
                <Row label={"Tax Proration Credit FROM Seller (reduces basis)"} field="taxAdjSeller" credit hint="Seller's unpaid property taxes credited to you. Look for 'County Taxes [date] to [date]' under Adjustments for Items Unpaid by Seller."/>
