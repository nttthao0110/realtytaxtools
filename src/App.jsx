    ...
    <div style={{maxWidth:600,margin:"0 auto"}}>
      ...
      {step===4&&res&&(<div>
        {res.nextQ&&res.nextQ.shortfall>0?(
          <div style={{background:"linear-gradient(135deg,#0d1a08,#0a1505)",border:"1px solid #1a4020",borderRadius:12,padding:"24px 20px",textAlign:"center",marginBottom:12}}>
            <div style={{fontSize:9,letterSpacing:"0.3em",textTransform:"uppercase",color:"#2a6030",marginBottom:8}}>{L==="vi"?"Thanh Toán Tiếp Theo":L==="es"?"Próximo Pago":"Your Next Payment Due"}</div>
            <div style={{fontSize:"clamp(36px,7vw,52px)",fontWeight:300,color:T.green,letterSpacing:"-0.02em",lineHeight:1}}>{fmt(res.nextQ.shortfall)}</div>
            <div style={{fontSize:13,color:"#3a8050",marginTop:8}}>{L==="vi"?"đến hạn":L==="es"?"vence":"due by"} <strong style={{color:T.gold}}>{res.nextQ.due}</strong></div>
            {res.catchUp>0&&<div style={{fontSize:11,color:"#5a7050",marginTop:4}}>({L==="vi"?"gồm bù đắp quý trước":L==="es"?"incluye recuperación":"includes catch-up"}: {fmt(res.catchUp)})</div>}
          </div>
        ):(
          <div style={{background:T.greenBg,border:"1px solid #1a4020",borderRadius:12,padding:"18px",textAlign:"center",marginBottom:12}}>
            <div style={{fontSize:20,marginBottom:4}}>✓</div>
            <div style={{fontSize:14,fontWeight:500,color:T.green}}>{L==="vi"?"Bạn đã nộp đủ!":L==="es"?"¡Ya pagó suficiente!":"You're fully paid up!"}</div>
          </div>
        )}

        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
          {[{l:L==="vi"?"Thuế Ước Tính":L==="es"?"Impuesto Est.":"Est. Tax",v:fmt(res.totalTax)},
            ...
            <div key={i} style={{background:T.bgCard,border:`1px solid ${it.hi?T.goldDim:T.border}`,borderRadius:8,padding:"10px 12px"}}>
          ...
          <div style={{overflowX:"auto",marginTop:8}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:420}}>
              <thead><tr style={{borderBottom:`1px solid ${T.border}`}}>
                {[L==="vi"?"Quý":"Qtr",L==="vi"?"Đáo Hạn":"Due",L==="vi"?"Cần Nộp":"Required","W-2",L==="vi"?"Đã Nộp":"Paid",L==="vi"?"Còn Thiếu":"Still Owe",""].map((h,i)=>(
                  <th key={i} style={{textAlign:"left",padding:"5px 4px 5px 0",color:T.textDim,fontWeight:400,fontSize:9,letterSpacing:"0.06em",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {res.quarters.map((q,i)=>(
                  <tr key={i} style={{borderBottom:`1px solid ${T.border}`}}>
                    ...
                    <td style={{padding:"8px 4px 8px 0",color:T.text}}>{fmt(q.required)}</td>
                    ...
                    <td style={{padding:"8px 4px 8px 0",color:!q.past?T.textDim:q.shortfall>0?T.red:T.green,fontSize:12}}>{!q.past?"→":q.shortfall>0?"⚠":"✓"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {res.totalPenalty>0&&<IBox tone="red">⚠ {L==="vi"?"Phạt ước tính":L==="es"?"Multa estimada":"Est. penalty"}: <strong>{fmt(res.totalPenalty)}</strong></IBox>}
        </Card>
        ...
        <Card>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>
            {res.quarters.filter(q=>!q.past&&q.shortfall>0).length===0
              ? <div style={{fontSize:13,color:T.green}}>✓ {L==="vi"?"Tất cả quý còn lại đã đủ!":L==="es"?"¡Todos cubiertos!":"All remaining quarters covered!"}</div>
              : res.quarters.map((q,i)=>{
                  if(q.shortfall===0&&q.past)return null;
                  return(
                    <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 14px",borderRadius:8,
                      ...
                      <div style={{textAlign:"right"}}>
                        <div style={{fontSize:17,color:q.past&&q.shortfall>0?T.red:q.shortfall>0?T.green:T.textDim,fontWeight:600}}>{q.shortfall>0?fmt(q.shortfall):"✓"}</div>
                        {q.shortfall>0&&<div style={{fontSize:10,color:T.textDim}}>{L==="vi"?"cần nộp":"to pay"}</div>}
                      </div>
                    </div>
                  );
                })
          ...
          <div style={{marginTop:10,padding:"10px 14px",background:T.bgCard2,borderRadius:7,fontSize:12,color:T.textMid,lineHeight:1.6}}>
            💡 <strong style={{color:T.gold}}>irs.gov/payments</strong> → {L==="vi"?"chọn 'Estimated Tax'":L==="es"?"seleccione 'Estimated Tax'":"select 'Estimated Tax'"} → {L==="vi"?"nhập số tiền trên":L==="es"?"ingrese el monto":"enter amount above"}
          </div>
        </Card>
